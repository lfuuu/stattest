...
<table class='timetable' cellspacing=0 border=0 id='timetable_pane' width='100%' align='center' {if !isset($timetableShow)}style='display:none'{/if}>
	...
	<tr>
		<td align='right'>&nbsp;</td>
		{section loop=24 start=9 name='myloop'}<td>{$smarty.section.myloop.index}</td>
		{/section}
		<td>&nbsp;</td>
		{section loop=24 start=9 name='myloop'}<td>{$smarty.section.myloop.index}</td>
		{/section}
				...
				<td style="{if $doer.time[$dates.key.yesterday].here eq $smarty.section.myloop.index}font-weight:bold;text-decoration:blink;{/if}">
					{if $doer.time[$dates.key.yesterday][$smarty.section.myloop.index]}
						...
						{else}
							...
							}blue{/if};text-decoration:none;" href="?module=tt&action=view&id={$d.trouble}">{$d.client}</a>&nbsp;{if $d.bill_no}<br><a href="./?module=newaccounts&action=bill_view&bill={$d.bill_no}">{$d.bill_no}</a>{/if}
                                {if $d.state_name}<br>{$d.state_name}{/if} {if $d.is_wrong}{$d.is_wrong}{/if}
                                {if $smarty.foreach.f_yesterday.total > 1 && !$smarty.foreach.f_yesterday.last}<hr>{/if}
                            {/foreach}
				...
				<td style="{if $doer.time[$dates.key.today].here eq $smarty.section.myloop.index}font-weight:bold;text-decoration:blink;{/if}">
					{if $doer.time[$dates.key.today][$smarty.section.myloop.index]}
						...
						{else}
							...
							}blue{/if};text-decoration:none;" target="_blank" href="?module=tt&action=view&id={$d.trouble}">{$d.client}</a>&nbsp;{if $d.bill_no}<br><a href="./?module=newaccounts&action=bill_view&bill={$d.bill_no}">{$d.bill_no}</a>{/if}
                                {if $d.state_name}<br>{$d.state_name}{/if} {if $d.is_wrong}{$d.is_wrong}{/if}
                                {if $smarty.foreach.f_today.total > 1 && !$smarty.foreach.f_today.last}<hr>{/if}
                            {/foreach}
						{/if}
					{elseif (!$flag_chck || $refix_flag) && $tt_id}
						<input type='checkbox' name='doer_fix[{$dates.key.today}][{$smarty.section.myloop.index}][{$doer_id}]' value='1' {*onclick='optools.tt.ctl_chckbxs(this)'*} />
					{else}
						&nbsp;
					{/if}
				</td>
			...
			{section loop=24 start=9 name='myloop'}
				<td style="{if $doer.time[$dates.key.tomorrow].here eq $smarty.section.myloop.index}font-weight:bold;text-decoration:blink;{/if}">
					{if $doer.time[$dates.key.tomorrow][$smarty.section.myloop.index]}
                        ...
                        {if $d.state_name}<br>{$d.state_name}{/if} {if $d.is_wrong}{$d.is_wrong}{/if}
                                {if $smarty.foreach.f_tomorrow.total > 1 && !$smarty.foreach.f_tomorrow.last}<hr>{/if}
                            {/foreach}
						{/if}
					{elseif (!$flag_chck || $refix_flag) && $tt_id}
						<input type='checkbox' name='doer_fix[{$dates.key.tomorrow}][{$smarty.section.myloop.index}][{$doer_id}]' value='1' {*onclick='optools.tt.ctl_chckbxs(this)'*} />
					{else}
						&nbsp;
					{/if}
				</td>
