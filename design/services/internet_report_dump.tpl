...
{foreach from=$ports item=item}<tr>
	...
	<td><b>{$item.port_type}, {if $item.port=='mgts'}{$item.node}{else}<a href='{$LINK_START}module=routers&id={$item.node}'>{$item.node}</a>::{$item.port}{/if}</b></td>
      ...
      <TABLE class=mform cellSpacing=4 cellPadding=2 width="100%" border=0>
        ...
        <TR>
        	...
        	<TD><SELECT name=port_type>
				<OPTION value=''{if $port_type==''} selected{/if}>все</OPTION>
				{foreach from=$port_types item=item}<option value='{$item}'{if $item==$port_type} selected{/if}>{$item}</option>{/foreach}
			</SELECT></TD>
		</TR>
        <TR>
