...
{/if}
	<TD width="1%" nowrap><a href="{$PATH_TO_ROOT}pop_services.php?table=usage_ip_ports&id={$conn.data.id}" target="_blank"><b>{$conn.data.id}</b></a>
                        ...
                        {if $conn.data.actual5d}<a href="{$LINK_START}module=services&action=in_act&id={$conn.data.id}" target="_blank"><img class=icon src='{$IMAGES_PATH}icons/act.gif' alt='Выписать акт'></a>{/if}
						...
						<a href='index.php?module=tt&clients_client={$conn.data.client}&service=usage_ip_ports&service_id={$conn.data.id}'><img class=icon src='{$IMAGES_PATH}icons/tt_new.gif' alt="Создать заявку"></a>
	</td>
...
<TD>{$cpe.actual_from}/{if !$cpe.actual}{$cpe.actual_to}{/if}</TD>
