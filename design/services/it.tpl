{if count($services_it) || !isset($is_secondary_output)}
{if !isset($is_secondary_output)}
<H2>Услуги</H2>
<H3>IT Park</H3>
{if access_action('services','it_add')}<a href='{$LINK_START}module=services&action=it_add'>Добавить услугу</a>{/if}
{else}
<H3><a href='?module=services&action=it_view'>IT Park</a></H3>
{/if}
<div border="1">
...
{foreach from=$services_it item=item name=outer}
<TR bgcolor="{if $item.status=='working'}{if $item.actual}#EEDCA9{else}#fffff5{/if}{else}#ffe0e0{/if}">
	...
	<td>{$item.price}</td>
	<td>
	{if $item.actual}<a href="{$LINK_START}module=services&action=it_close&id={$item.id}"><img class=icon src='{$IMAGES_PATH}icons/delete.gif' alt="Отключить"></a>{/if}
		<a href='index.php?module=tt&clients_client={$item.client}&service=usage_extra&service_id={$item.id}&action=view_type&type_pk=2&show_add_form=true'><img class=icon src='{$IMAGES_PATH}icons/tt_new.gif' alt="Создать заявку"></a>
	</td>
</tr>	
{/foreach}
</tbody>
