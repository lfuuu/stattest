...
{/if}{/foreach}
    ...
    <td>{if $qrs[$op.bill.bill_no].11}<a {if access('newaccounts_akts','delete')}class="del_doc"{/if} id="{$qrs[$op.bill.bill_no].11}" href="./?module=newaccounts&action=doc_file&id={$qrs[$op.bill.bill_no].11}" target=_blank title="Акт-1"><img border=0 src="images/icons/act.gif"></a>{else}&nbsp;{/if}</td>
    <td>{if $qrs[$op.bill.bill_no].12}<a {if access('newaccounts_akts','delete')}class="del_doc"{/if} id="{$qrs[$op.bill.bill_no].12}" href="./?module=newaccounts&action=doc_file&id={$qrs[$op.bill.bill_no].12}" target=_blank title="Акт-2"><img border=0 src="images/icons/act.gif"></a>{else}&nbsp;{/if}</td>
	{if $pay.comment}
