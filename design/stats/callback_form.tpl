      <H3>Создайте отчёт сами: (или - посмотрите отчёты за <a href="?module=stats&ip={$ip}&action=callback&detality=day&from_d={$prev_from_d}&to_d={$prev_to_d}&from_m={$prev_from_m}&to_m={$prev_to_m}&from_y={$prev_from_y}&to_y={$prev_to_y}">прошлый месяц</a>,
      								за <a href="?module=stats&ip={$ip}&action=callback&detality=day&from_d={$cur_from_d}&to_d={$cur_to_d}&from_m={$cur_from_m}&to_m={$cur_to_m}&from_y={$cur_from_y}&to_y={$cur_to_y}">текущий месяц</a>)</H3>
