...
class m160211_172500_convert_voip_package_account_tariff extends \app\classes\Migration
{
  ...
  SELECT usage_voip_package.id + {$deltaVoipPackageAccountTariff},
      clients.id,
  ...
  WHERE usage_voip_package.client = clients.client
    ...
    public function convertAccountTariffLog()
    {
        ...
        $serviceTypeIdVoipPackage = ServiceType::ID_VPBX;
  ...
  SELECT log_tarif.date_activation, usage_voip_package.id + {$deltaVoipAccountTariffPackage}, {$tariffPeriodTableName}.id,
  ...
  WHERE log_tarif.service = 'usage_voip_package'
    ...
    (
        SELECT COUNT(*)
        FROM log_tarif
        WHERE service = 'usage_voip_package'
    ) AS count_old,
    ...
    (
        ...
        AND {$accountTariffTableName}.service_type_id = {$serviceTypeIdVoipPackage}
    ) AS count_new
    ")->queryOne();
