...
function work()
{
    ...
    $didGroups = [];
    foreach ($db->AllRecords("select id, city_id, beauty_level from did_group order by city_id, beauty_level, id") as $row) {
        if (!isset($didGroups[$row['city_id']])) {
            $didGroups[$row['city_id']] = [];
        }
        if (!isset($didGroups[$row['city_id']][$row['beauty_level']])) {
            $didGroups[$row['city_id']][$row['beauty_level']] = $row['id'];
        }
    };
    ...
    foreach($nums as $num) {
        ...
        if (isset($didGroups[$num['city_id']][$beautyLevel])) {
        ...
        if ($didGroupId) {
            ...
            echo $num['number'].' - '.$beautyLevel." ERROR Did group not found<br/>\n";
            $errorsCount++;
        }
    }
