...
//        $this->_addInheritance(new m_stats_);
    ...
    function stats_ppp( $fixclient )
    {
        ...
        if ( $login )
        {
            $db->Query( 'select * from usage_ip_ppp where (client="' .
                        $clientNick .
                        '") and (id="' .
                        $login .
                        '")' );
            if ( ! ( $r = $db->NextRecord() ) )
            {
                trigger_error2( 'Логин не существует' );
    ...
    function stats_voip( $fixclient )
    {
        ...
        foreach ( $usages as $r )
        {
            ...
            {
                if ( ! isset( $regions[ $r[ 'region' ] ] ) ) $regions[ $r[ 'region' ] ] = [ ];
                if ( ! isset( $regions[ $r[ 'region' ] ][ $r[ 'id' ] ] ) ) $regions[ $r[ 'region' ] ][ $r[ 'id' ] ] =
                    $r[ 'id' ];
            }
            if ( substr( $r[ 'phone_num' ], 0, 4 ) ==
                '7095'
            ) $r[ 'phone_num' ] =
                '7495' .
                substr( $r[ 'phone_num' ], 4 );
            if ( $last_region !=
                $r[ 'region' ]
            )
            {
        ...
        if ( ! in_array( $destination, [ 'all', '0', '0-m', '0-f', '1', '1-m', '1-f', '2', '3' ] ) )
        {
            $destination = 'all';
        }

        $direction = get_param_raw( 'direction', 'both' );
        if ( ! in_array( $direction, [ 'both', 'in', 'out' ] ) )
        {
            $direction = 'both';
        }
        ...
        )
        {
            ...
            {
                $stats[ $region ] =
                    $this->GetStatsVoIP( $region, $from, $to, $detality, $account->id, $phones_sel, $paidonly, 0, $destination, $direction, $timezone, $regions );
            }
    ...
    */
    function prepareStatArray( ClientAccount $account, $data = [ ], $detality = '', $all_regions = [ ] )
    {

        if ( ! count( $data ) ) return $data;
        $Res = [ ];
        $rt = [ 'price' => 0, 'cnt' => 0, 'ts2' => 0, 'len' => 0, 'price_with_tax' => 0, 'price_without_tax' => 0 ];
        ...
        switch ( $detality )
        {
            case 'dest':
                    ...
                    foreach ( $reg_data as $k => $r )
                    {
                        ...
                        )
                        {
                            if ( ! isset( $Res[ $k ] ) ) $Res[ $k ] =
                                [ 'tsf1' => $r[ 'tsf1' ], 'reg_id' => $r_id, 'cnt' => 0, 'price' => 0, 'len' => 0 ];
                ...
                {
                    $rt[ 'price_without_tax' ] =
                        number_format( $rt[ 'price' ] *
                                       100 /
                                       ( 100 +
                                           $tax_rate ), 2, '.', '' );
                ...
                foreach ( $data as $r_id => $reg_data )
                {
                    foreach ( $reg_data as $r )
                    {
                        ...
                        )
                        {
                            ...
                            $Res[] =
                                [ 'mktime' => $r[ 'mktime' ], 'reg_id' => ( isset( $all_regions[ $r_id ] ) ? $all_regions[ $r_id ] : $r_id ) ] +
                                $r;
                ...
                foreach ( $data as $r_id => $reg_data )
                {
                    foreach ( $reg_data as $k => $r )
                    {
                        ...
                        )
                        {
                            ...
                            $Res[ $r[ 'ts1' ] ][ 'price' ] =
                                number_format( $Res[ $r[ 'ts1' ] ][ 'price' ], 2, '.', '' );
    ...
    function GetStatsInternet( $client, $from, $to, $detality, $routes, $is_collocation = 0 )
    {
        ...
        {
            $r = [ 'in_bytes' => 0, 'out_bytes' => 0, 'ts' => 0, 'tsf' => 0 ];
...
//printdbg($routes);
        ...
        )
        {
            $W =
                [
                    'AND', $P, 'router="rubicon"', 'time>=FROM_UNIXTIME(' .
                    $from .
                    ')',
                ];
            if ( $to <
            ...
            else
            {
                $db->Query( $q = "
					select
						...
						" .
                    ( $group ? 'sum' : '' ) .
                    "(in_r+in_r2+in_f) as in_bytes,
						" .
                    ( $group ? 'sum' : '' ) .
                    "(out_r+out_r2+out_f) as out_bytes,
						UNIX_TIMESTAMP(time) as ts
					from
						$tbl
					where
            ...
            if ( $countRow ==
                5000
            ) trigger_error2( 'Статистика отображается не полностью. Сделайте ее менее детальной или сузьте временной период' );
        }
        $T[ 'ts' ] = '<b>Итого</b>';
        $T[ 'tsf' ] = '<b>Итого</b>';
        $T[ 'ip' ] = '&nbsp;';
    ...
    function GetStatsVPN( $client, $from, $to, $detality, $IPs )
    {
        ...
        $R = [ ];
        $rt = [ 'in_bytes' => 0, 'out_bytes' => 0 ];
        if ( $whsql )
        {
            $whsql =
                '(datetime>=FROM_UNIXTIME(' .
                $from .
                ') AND datetime<FROM_UNIXTIME(' .
                $to .
                '+86400)) AND (' .
                $whsql .
                ')';
            $db->Query( "select inet_ntoa(ip_int) as ip,sum(transfer_rx) as in_bytes,sum(transfer_tx) as out_bytes,UNIX_TIMESTAMP(datetime) as ts from $tbl where " .
                        $whsql .
                        $group .
                        " ORDER by " .
                        $order .
    ...
    function FindByNumber( $region, $from, $to, $find, $timezone = "Europe/Moscow" )
    {
        ...
        {
            $l[ "time" ] =
                mdate( "d месяца Y г. H:i:s", strtotime( $l[ "connect_time" ] ) +
                                            $offset );
            ...
            if ( isset( $l[ 'geo_id' ] ) )
            {
                ...
                {
                    $geo[ $l[ 'geo_id' ] ] =
                        $pg_db->GetValue( 'select name from geo.geo where id=' .
                                          ( (int) $l[ 'geo_id' ] ) );
                }
    ...
    function GetStatsVoIP( $region, $from, $to, $detality, $client_id, $usage_arr, $paidonly = 0, $skipped = 0, $destination = 'all', $direction = 'both', $timezone = "Europe/Moscow", $regions = [ ], $isFull = false )
    {
        global $pg_db;
        ...
        )
        {
            ...
            {
                $sql .= " date_trunc('day',connect_time + '{$offset} second'::interval) as ts1, ";
            }
            elseif ( $detality ==
                'month'
            ) $sql .= " date_trunc('month',connect_time + '{$offset} second'::interval) as ts1, ";
            elseif ( $detality ==
                'year'
            ) $sql .= " date_trunc('year',connect_time + '{$offset} second'::interval) as ts1, ";
            ...
            $sql .=
                ...
                ( $group ? 'sum' : '' ) .
                '(' .
                ( $paidonly ? 'case abs(cost)>0.0001 when true then billed_time else 0 end' : 'billed_time' ) .
                ') as ts2,
                                    ' .
                ( $group ? 'sum(' .
                    ( $paidonly ? 'case abs(cost)>0.0001 when true then 1 else 0 end' : 1 ) .
                    ')' : '1' ) .
                ' as cnt
                            from
                                    calls_raw.calls_raw
        ...
        else
        {
            ...
            $R = [
                'mos_loc' => [ 'tsf1' => 'Местные Стационарные', 'cnt' => 0, 'len' => 0, 'price' => 0, 'is_total' => false ],
                'mos_mob' => [ 'tsf1' => 'Местные Мобильные', 'cnt' => 0, 'len' => 0, 'price' => 0, 'is_total' => false ],
                'rus_fix' => [ 'tsf1' => 'Россия Стационарные', 'cnt' => 0, 'len' => 0, 'price' => 0, 'is_total' => false ],
                'rus_mob' => [ 'tsf1' => 'Россия Мобильные', 'cnt' => 0, 'len' => 0, 'price' => 0, 'is_total' => false ],
                'int'     => [ 'tsf1' => 'Международка', 'cnt' => 0, 'len' => 0, 'price' => 0, 'is_total' => false ],
            ];
            //$db_calls->Query($sql);
            $pg_db->Query( $sql );
            //while ($r=$db_calls->NextRecord()){
    ...
    function GetStatsCallback( $from, $to, $detality, $client_id )
    {
        ...
        $sql = "select C.*,PA.phone_num as num_from,PB.phone_num as num_to," .
            $groupQ .
            "(A.tarif_sum) as priceFrom," .
            $groupQ .
            "(B.tarif_sum) as priceTo," .
            $groupQ .
            "(A.tarif_sum+B.tarif_sum) as price," .
            "UNIX_TIMESTAMP(ts) as ts1," .
            $groupQ .
            "(B.lengthResult) as ts2 " .
            ...
            $whsql .
            $group .
            " ORDER by C.ts ASC LIMIT 5000";
        $db->Query( $sql );
        $rt = [ 'price' => 0, 'priceFrom' => 0, 'priceTo' => 0, 'ts2' => 0 ];
    ...
    function GetStatsPPP( $from, $to, $detality, $logins )
    {
        ...
        $whsql =
            ...
            $to .
            '+86400))';
        if ( ! count( $logins ) ) return [ ];
        if ( count( $logins ) ==
            1
        )
        {
            foreach ( $logins as $r ) $whsql .= ' AND (UserName="' .
                $r .
                '")';
        }
        else
        {
            $whsql .= '';
            foreach ( $logins as $k => $r ) $logins[ $k ] =
                '"' .
                $r .
                '"';
            $p = implode( ',', $logins );
            $whsql .= ' AND (UserName IN (' .
                $p .
                '))';
        }
        ...
        $sql = "select UserName as login," .
            ...
            ( $group ? 'sum' : '' ) .
            "(AcctOutputOctets) as out_bytes," .
            "UNIX_TIMESTAMP(AcctStartTime) as ts1," .
            ( $group ? 'sum' : '' ) .
            "(AcctSessionTime) as ts2 " .
            "from radacct where " .
            $whsql .
            $group .
            " ORDER by AcctStartTime ASC LIMIT 5000";
        $db->Query( $sql );
        $rt = [ 'ts2' => 0, 'in_bytes' => 0, 'out_bytes' => 0 ];
    ...
    {
        global $db;
        $R = [ ];// and usage_ip_ports.trafcounttype="flows"
        $db->Query( 'select usage_ip_routes.* from usage_ip_routes left join usage_ip_ports on usage_ip_ports.id=usage_ip_routes.port_id where usage_ip_ports.client="' .
                    $client .
                    '" order by usage_ip_routes.net' );
        $i = 0;
        while ( $r = $db->NextRecord() )
        {
            $i++;
    ...
    function get_routes_list_ip( $routes_allB )
    {
        ...
        foreach ( $routes_allB as $k => $R )
        {
            ...
            $sum ; $i++ )
            {
                $routes_all[ "{$n[1]}.{$n[2]}.{$n[3]}.{$n[4]}" ] =
                    [ "{$n[1]}.{$n[2]}.{$n[3]}.{$n[4]}", $R[ 1 ], $R[ 2 ] ];
                $n[ 4 ]++;
                if ( $n[ 4 ] >=
                    256
                )
    ...
    function get_routes_list( $client )
    {
        ...
        while ( $r = $db->NextRecord() )
        {
            if ( $r[ 'net' ] )
            {
                $routes_all[ $r[ 'net' ] ] = [ $r[ 'net' ], $r[ 'actual_from' ], $r[ 'actual_to' ] ];
            }
        }
    ...
    function get_client_stats( $client, $from, $to )
    {
        ...
        return [ 'in_bytes' => $r[ 'in_bytes' ], 'out_bytes' => $r[ 'out_bytes' ] ];
    }
    ...
    function stats_send_add( $fixclient )
    {
        ...
        foreach ( $clients as $i => $c )
        {
            ...
            {
                @$db->Query( 'insert into stats_send (client,state,year,month,port_id,bytes,max_bytes,message) value ("' .
                             $c .
                             '","ready",' .
                             $year .
                             ',' .
    ...
    function stats_send_process( $fixclient )
    {
        ...
        $C = [ ];
        while ( $r = $db->NextRecord() ) $C[ $r[ 'client' ] ] = $r[ 'client' ];
        foreach ( $C as $client )
        {
        ...
        $db->Query( 'select stats_send.*, c.id AS clientid,' .
                    $q .
                    ' as cur_sent from stats_send
    ...
    {
        global $db;
        $db->Query( 'select *,ROUND(bytes/(1024*1024),1) as mbytes,ROUND(max_bytes/(1024*1024),1) as mmax from stats_send where (client="' .
                    $client .
                    '") and (!last_send || (last_send+INTERVAL 1 DAY < NOW())) AND (state!="sent") group by client order by state,last_send desc,client LIMIT 5' );
        $R = [ ];
        ...
        if ( ( defined( 'MAIL_TEST_ONLY' ) &&
                ( MAIL_TEST_ONLY ==
                    1 ) ) ||
            $is_test
        ) $C[ 'email' ] = 'andreys75@mcn.ru, shepik@yandex.ru';
        ...
        if ( $C[ 'email' ] &&
            ( mail( $C[ 'email' ], $subj, $body, $headers ) )
        )
        {
            $db->Query( 'update stats_send set state="sent",last_send=NOW(),message="' .
                        $msg .
                        '" where (client="' .
                        $client .
                        '") and (state!="sent")' );
        ...
        {
            $db->Query( 'update stats_send set state="error",last_send=NOW(),message="' .
                        $msg .
    ...
    function stats_report_traff_less()
    {
        ...
        if ( get_param_raw( "make_report", "false" ) !=
            "false"
        )
        {
            $manager_inj =
                ( $manager <>
                    'anyone' ) ? 'AND c.manager = \'' .
                    addcslashes( $manager, "'\\" ) .
                    '\' ' : '';
            $q = "
				SELECT
    ...
    function stats_report()
    {
        ...
        )
        {
            $date_sql_filter = "tfr.date BETWEEN '" .
                ...
                "' and '" .
                date( 'Y-m-d', mktime( 0, 0, 0, ( (int) $_GET[ 'm' ] ==
                                 12 ) ? 1 : (int) $_GET[ 'm' ] +
                                 1, 1, ( (int) $_GET[ 'm' ] ==
                                 12 ) ? (int) $_GET[ 'y' ] +
                                 1 : (int) $_GET[ 'y' ] ) -
        ...
        $isTrafLess =
            get_param_raw( 'is_traf_less', "nan" ) ==
            "true";
        $trafLess = (float) ( get_param_raw( 'traf_less', 10 ) );
        ...
        )
        {
             ...
             */
            if ( $show_unlim )
            { // показывать ли безлимитные тарифы
            ...
            if ( $isOver )
            { // превышение трафика в "небезлимитных" тарифах
                $flags[ 'over_flag' ] = "
						(
							((sum(tfr.in_bytes)+sum(tfr.out_bytes))/(1024*1024)) > 10
						AND
							(((sum(tfr.in_bytes)+sum(tfr.out_bytes))/(1024*1024)) * " .
                    $over .
                    ") > ti.mb_month
						AND
            ...
            $query_total = "
                ...
                $date .
                "','%Y-%m-1') and date_format('" .
                $date .
                "','%Y-%m-1')+interval 1 month-interval 1 day
				group by
					id_port";
        ...
        if ( isset( $_GET[ 'show_tarif_traf' ] ) &&
            $_GET[ 'show_tarif_traf' ] ==
            'true'
        )
        {
    ...
    function stats_report_services()
    {
        ...
        $def_statuses =
            [ 'work', 'negotiations', 'testing', 'connecting', 'debt', 'suspended', 'operator', 'distr', 'blocked' ];
    ...
    function stats_report_sale_channel( $fixclient )
    {
        ...
        $design->assign( [ 'date_begin' => $date_begin, 'date_end' => $date_end ] );
        ...
        while ( $row = $db->NextRecord( MYSQL_ASSOC ) )
        {
            ...
            $bonus = 0;
            $row[ "bill_sum" ] =
            $row[ "sum_good" ] = $row[ "sum_service" ] = $row[ "count_service" ] = $row[ "count_good" ] = 0;
        ...
        else
        {
            ...
            $design->assign(
                ...
                array_merge(
                    [
                        [ 'id' => 'null', 'name' => 'Все (кроме: закрыт, отказ)' ],
                    ],
                    $db->AllRecords( "
                        SELECT
                            `id`,
    ...
    function stats_agent_settings( $fixclient )
    {
        ...
        $sale_channel =
            $db->GetValue( 'SELECT name FROM sale_channels_old WHERE is_agent = 1 AND dealer_id = ' .
                           $fixclient_data[ 'id' ] );
    ...
    function stats_save_agent_settings( $fixclient )
    {
        ...
        $interests = get_param_raw( 'interest', [ ] );
        ...
        {
            $data = [ 'client_id' => $fixclient_data[ 'id' ], 'interest' => $interest ];
            foreach ( $interests as $k => $v )
            {
                $data[ $k ] = $v;
            }
    ...
    function stat_test( $itemNum, $dateFrom, $dateTo )
    {
        ...
        $vv = $db->AllRecords( "select l.bill_no, cast(s.date_start as date) as ds,  client
            ...
            where s.date_start between '" .
                               $dateFrom .
                               "' and '" .
                               $dateTo .
                               "'  and s.trouble_id = t.id and s.state_id = 18 and t.bill_no = l.bill_no and
            item_id =g.id and  g.num_id = " .
                               $itemNum .
                               " and b.bill_no = l.bill_no and client ='WiMaxComstar'
    ...
    function make_calend( $date, $data )
    {
        ...
        $days ; $i++ )
        {
            ...
            {
                $o .= "<tr><td style='font: normal 6pt sans-serif;'>" .
                    $pId .
                    "</td><td>&nbsp;";
                if ( isset( $pData[ "data" ][ $_dd ] ) )
                {
                    $v = $pData[ "data" ][ $_dd ];
                    $o .= ( $v[ "income" ] ? "<b style='color:red;'>+" .
                        $v[ "income" ] .
                        "</b>" : "" );
                    $o .= "</td><td>&nbsp;";
                    $o .= $v[ "outlay" ] ? "<b style='color:green;'>-" .
                        $v[ "outlay" ] .
                        "<b>" : "";
                }
                else
...
#where order_given = 'WiMAX COMSTAR'
            ...
            and (
                    (ts.state_id in(2,20,21) and date_edit between '" .
                                  $d1 .
                                  " 00:00:00' and '" .
                                  $d2 .
                                  " 23:59:59')
        ...
        if ( $genReport )
        {
            return [ $date, $design->fetch( "stats/wimax.html", null, null, false ) ];
        }
        else
        {
    ...
    function stats_courier_sms( $fixclient, $genReport = false )
    {
        ...
        $r = $db->AllRecords(
            ...
            where sms_send between '" .
            $d1 .
            " 00:00:00' and '" .
            $d2 .
            " 23:59:59' group by sms_sender" );
        ...
        if ( $genReport )
        {
            return [ $date, $design->fetch( "stats/courier_sms.html", null, null, false ) ];
        }
        else
        {
    ...
    function stats_support_efficiency( $fixclient )
    {
        ...
        $dateFrom = $date_from->getDay();
        $dateTo = $date_to->getDay();
        $date =
            $dateFrom ==
            $dateTo ? 'за ' .
                $dateFrom : 'с ' .
        ...
        $onCompleted_users2 = $onCompleted_data2 = $onCompleted_total2 = $onCompleted_rating2 = [ ];

        if ( get_param_raw( "make_report", "" ) ==
            "OK"
        )
        {
            ...
            foreach ( $r as $l )
            {
                ...
                $m[ $l[ "user_author" ] ][ "data" ][ $l[ "trouble_subtype" ] ] = [
                    "count"        => $l[ "c" ],
                    "rating_avg"   => ( $l[ "rating_count" ] >
                    0 ? $l[ "rating" ] /
                        $l[ "rating_count" ] : 0 ),
            ...
            list( $onCompleted_data, $onCompleted_users, $onCompleted_total, $onCompleted_rating ) =
                $this->stats_support_efficiency__basisOnCompleted( $dateFrom, $dateTo, $usage );
            list( $onCompleted_data2, $onCompleted_users2, $onCompleted_total2, $onCompleted_rating2 ) =
                $this->stats_support_efficiency__basisOnStartDate( $dateFrom, $dateTo, $usage );
        }
    ...
    function stats_support_efficiency__basisOnStartDate( &$dateFrom, &$dateTo, &$usage )
    {
        ...
        foreach ( $rs as $r )
        {
            if ( $r[ "state_id" ] ==
                7 &&
                strlen( $r[ "user_rating" ] )
            )
            {
                if ( ! isset( $tmp[ $r[ "trouble_id" ] ] ) )
                {
                    $tmp[ $r[ "trouble_id" ] ] =
                        [ 'type' => $r[ "type" ], 'user_rating' => '', '7' => 0, '2' => 0, '1u' => [ ] ];
                }
                ...
                {
                    if ( strlen( $tmp[ $r[ "trouble_id" ] ][ 'user_rating' ] ) &&
                        $tmp[ $r[ "trouble_id" ] ][ 'user_rating' ] !=
                        $r[ "user_rating" ]
                    )
                    {
        ...
        foreach ( $tmp as $k => $rat )
        {
            if ( strlen( $rat[ 'user_rating' ] ) )
            {
                ...
                {
                    $rating[ $rat[ 'user_rating' ] ] = [ ];
                }
                if ( ! isset( $rating[ $rat[ 'user_rating' ] ][ $rat[ 'type' ] ] ) )
                {
                    $rating[ $rat[ 'user_rating' ] ][ $rat[ 'type' ] ] = [ '7' => 0, '2' => 0 ];
                }

                $rating[ $rat[ 'user_rating' ] ][ $rat[ 'type' ] ][ '7' ] += $rat[ '7' ];
                $rating[ $rat[ 'user_rating' ] ][ $rat[ 'type' ] ][ '2' ] += $rat[ '2' ];

                if ( ! isset( $users[ $rat[ 'user_rating' ] ] ) ) $users[ $rat[ 'user_rating' ] ] =
                    $rat[ 'user_rating' ];
            }
        }

        foreach ( $db->AllRecords( "select user, name from user_users where user in ('" .
                                   implode( "','", $users ) .
                                   "')" ) as $u )
        {
            $users[ $u[ "user" ] ] = $u[ "name" ];
        ...
        return [ $counter, $users, $total, $rating ];
    }
    ...
    function stats_support_efficiency__basisOnCompleted( &$dateFrom, &$dateTo, &$usage )
    {
        ...
        foreach ( $rs as $r )
        {
            ...
            {
                $tmp[ $r[ "trouble_id" ] ] =
                    [ 'type' => $r[ "type" ], 'user_author' => $r[ "user_author" ], 'user_7' => '', 'user_2' => '', 'user_rating' => '', '7' => 0, '2' => 0, '1u' => [ ] ];
            }
        ...
        foreach ( $tmp as $trouble_id => $t )
        {
            ...
            if ( strlen( $t[ 'user_7' ] ) )
            {
                if ( ! isset( $counter[ '7' ][ $t[ 'type' ] ] ) ) $counter[ '7' ][ $t[ 'type' ] ] = [ ];
            ...
            if ( strlen( $t[ 'user_2' ] ) )
            {
                if ( strlen( $t[ 'user_7' ] ) )
                {
                    ...
                    if ( ! isset( $counter[ '2' ][ $t[ 'type' ] ] ) ) $counter[ '2' ][ $t[ 'type' ] ] = [ ];
                ...
                else
                {
                    ...
                    if ( ! isset( $counter[ '2w7' ][ $t[ 'type' ] ] ) ) $counter[ '2w7' ][ $t[ 'type' ] ] = [ ];
            ...
            if ( strlen( $t[ 'user_rating' ] ) )
            {
                ...
                {
                    $rating[ $t[ 'user_rating' ] ][ $t[ 'type' ] ] = [ '7' => 0, '2' => 0 ];
                }
    ...
    function stats_report_plusopers( $fixclient, $client, $genReport = false, $viewLink = true )
    {
        ...
        if ( in_array( get_param_raw( "do", "" ), [ "add", "edit" ] ) )
        {
            $error = "";
            $info = "";
            $a = [ "bill_no" => get_param_raw( "bill_no", "" ) ];

            if ( get_param_raw( "info_form", "" ) )
            {
                $db->QueryUpdate( "params", "param", [
                                              "param" => "onlime_info",
            ...
            if ( get_param_raw( "form", "" ) )
            {
                ...
                if ( ! $error )
                {
                    ...
                    $i =
                        $db->GetRow( "select * from newbills_add_info where bill_no = '" .
                                     $db->escape( $a[ "bill_no" ] ) .
                                     "'" );
                    ...
                    {
                        $tId =
                            $db->GetValue( "select id from tt_troubles where bill_no = '" .
                                           $a[ "bill_no" ] .
                                           "'" );
                        $curStageId =
                            $db->GetValue( "select cur_stage_id from tt_troubles where bill_no = '" .
                                           $a[ "bill_no" ] .
                                           "'" );
                        $stateId =
                            $db->GetValue( "select state_id from tt_stages where stage_id = '" .
                                           $curStageId .
                                           "'" );
                        ...
                        $tsid = $db->QueryInsert( 'tt_stages', [
                                                                 'trouble_id'          => $tId,
                                                                 'state_id'            => $stateId,
                                                                 'user_main'           => $client,
                                                                 'date_start'          => [ 'now()' ],
                                                                 'date_finish_desired' => [ 'now()' ],
        ...
        else
        {
            ...
            $filterPromoAll = [ "all" => "Все заявки", "promo" => "По акции", "no_promo" => "Не по акции" ];
        ...
        {
            list( $r1, $closeList1, $deliveryList1 ) =
                $this->report_plusopers__getCount( "onlime", $d1, $d2, $filterPromo );
            list( $r2, $closeList2, $deliveryList2 ) =
                $this->report_plusopers__getCount( "onlime2", $d1, $d2, $filterPromo );
        ...
        {
            list( $r, $closeList, $deliveryList ) =
                $this->report_plusopers__getCount( $client, $d1, $d2, $filterPromo );
        }
        ...
        )
        {
            ...
            {
                $list1 =
                    $this->report_plusopers__getList( "onlime", $listType, $d1, $d2, $deliveryList1, $closeList1, $filterPromo );
                $list2 =
                    $this->report_plusopers__getList( "onlime2", $listType, $d1, $d2, $deliveryList2, $closeList2, $filterPromo );
            ...
            {
                $list =
                    $this->report_plusopers__getList( $client, $listType, $d1, $d2, $deliveryList, $closeList, $filterPromo );
            }
        }
        ...
        )
        {
            ...
            $sTypes = [
                "work"     => [
                    ...
                    "title" => ( $client ==
                    "nbn" ? "в работе" : "В Обработке" ),
                ],
                "close"    => [
                    "sql"   => "is_rollback =0 and state_id in (2,20)",
                    "title" => ( $client ==
                    "nbn" ? "закрытые" : "Доставлен" ),
                ],
                "reject"   => [
            ...
            $this->GenerateExcel( "OnLime__" .
                                  str_replace( " ", "_", $sTypes[ $listType ][ "title" ] ) .
                                  "__" .
                                  $d1 .
                                  "__" .
                                  $d2,
    ...
    private function report_plusopers__addressToStr( $l )
    {
        ...
        {
            list( $street, $home, $bild, $porch, $floor, $flat, $intercom ) =
                explode( " ^ ", $l[ "address" ] .
                              " ^  ^  ^  ^  ^  ^  ^  ^ " );
            $a = $street;
            if ( $home ) $a .= ", д." .
    ...
    private function report_plusopers__getFields_and_check( $client, $a )
    {
        ...
        if ( isset( $fields[ "date_deliv" ] ) &&
            ! $error &&
            ! strtotime( $a[ "date_deliv" ] )
        )
        {
        ...
        )
        {
            ...
            foreach ( [
                          "address_street", "address_house", "address_building", "address_porch",
                          "address_floor", "address_flat", "address_interkom",
                      ] as $f )
            {
                $a[ $f ] = get_param_raw( $f, "" );
                $address .= $a[ $f ] .
            ...
            foreach ( [ "phone_home", "phone_mobil", "phone_work" ] as $f )
            {
                $a[ $f ] = get_param_raw( $f, "" );
                $phone .= $a[ $f ] .
                    " ^ ";
    ...
    private function report_plusopers__Load( $client, $billNo )
    {
        ...
        )
        {
            $a[ "req" ] = $a[ "req_no" ];
            $a[ "comment" ] =
                $db->GetValue( "select comment from newbills where bill_no = '" .
                               $db->escape( $billNo ) .
                               "'" );
            ...
            {
                $a[ "qty" ] =
                    $db->GetValue( "select amount from newbill_lines where bill_no = '" .
                                   $billNo .
                                   "' and item_id = '4e8cc21b-d476-11e0-9255-d485644c7711'" );
                $a[ "deliv_type" ] =
                    $db->GetValue( "select amount from newbill_lines where bill_no = '" .
                                   $billNo .
                                   "' and item_id = 'a449a3f7-d918-11e0-bdf8-00155d21fe06'" ) ? "moskow" : "mkad";
            }
            else
            {//onlime
                $a[ "qty" ] =
                ...
                $a[ "deliv_type" ] =
                    $db->GetValue( "select amount from newbill_lines where bill_no = '" .
                                   $billNo .
                                   "' and item_id = '81d52242-4d6c-11e1-8572-00155d881200'" ) ? "moskow" : "mkad";
                ...
                foreach ( [ "phone_home", "phone_mobil", "phone_work" ] as $idx => $f )
                {
                    $a[ $f ] = @$phone[ $idx ];
                }
            }
    ...
    private function report_plusopers__getCount( $client, $d1, $d2, $filterPromo )
    {
        ...
        return [ $r, $closeList, $deliveryList ];
    }
    ...
    private function report_plusopers__getList( $client, $listType, $d1, $d2, $deliveryList, $closeList, $filterPromo )
    {
        ...
        {
            $sTypes[ "rollback" ] = [ "sql" => "is_rollback =1", "title" => "Возврат" ];
        }
    ...
    public function stats_report_phone_sales()
    {
        ...
        while ( mktime( 0, 0, 0, $tmp_m, 1, $tmp_y ) >=
            mktime( 0, 0, 0, $from_m, 1, $from_y ) ) // process all monthes in interval
        {
            $tmp_date_from =
                $tmp_y .
                '-' .
            ...
            $res = $db->AllRecords( "
                select
                    ...
                    u.no_of_lines,
                    c.id as client_id,
                    ifnull(c.created >= date_add('" .
                                    $tmp_date_from .
                                    "',interval - 1 month), 0) as is_new,
            ...
            $sale_nums = [ 'all' => [ 'new' => 0, 'old' => 0, 'all' => 0 ] ];
            $sale_8800 = [ 'all' => [ 'new' => 0, 'old' => 0, 'all' => 0 ] ];
            $sale_vpbx = [ 'all' => [ 'new' => 0, 'old' => 0, 'all' => 0 ] ];
            $sale_nonums = [ 'all' => [ 'new' => 0, 'old' => 0, 'all' => 0 ] ];
            $sale_lines = [ 'all' => [ 'new' => 0, 'old' => 0, 'all' => 0 ] ];
            $sale_clients = [ 'all' => [ 'new' => 0, 'old' => 0, 'all' => 0 ] ];
            $sale_channels =
                [ 'all' => [ 'vpbx' => [ 'new' => 0, 'old' => 0, 'all' => 0 ], 'nums' => [ 'new' => 0, 'old' => 0, 'all' => 0 ], 'lines' => [ 'new' => 0, 'old' => 0, 'all' => 0 ], 'visits' => 0 ], "managers" => [ ] ];
            $clients = [ ];
            $clients_vpbx = [ ];
            $vpbx_clients = [ 'all' => [ 'new' => 0, 'old' => 0, 'all' => 0 ] ];
            ...
            foreach ( $res as $r )
            {
                ...
                {
                    if ( strpos( $r[ 'phone' ], '7800' ) ===
                        0
                    )
                    {
                        if ( ! isset( $sale_8800[ $r[ 'region' ] ] ) )
                        {
                            $sale_8800[ $r[ 'region' ] ] = [ 'new' => 0, 'old' => 0, 'all' => 0 ];
                        }
                        if ( $r[ 'is_new' ] >
                            0
                        )
                    ...
                    else
                    {
                        ...
                        {
                            $sale_nums[ $r[ 'region' ] ] = [ 'new' => 0, 'old' => 0, 'all' => 0 ];
                        }
                        if ( $r[ 'is_new' ] >
                            0
                        )
                ...
                { //линия без номера
                    ...
                    {
                        $sale_nonums[ $r[ 'region' ] ] = [ 'new' => 0, 'old' => 0, 'all' => 0 ];
                    }
                ...
                {
                    $sale_lines[ $r[ 'region' ] ] = [ 'new' => 0, 'old' => 0, 'all' => 0 ];
                }
                ...
                if ( ! isset( $clients[ $r[ 'client_id' ] ] ) )
                {
                    ...
                    {
                        $sale_clients[ $r[ 'region' ] ] = [ 'new' => 0, 'old' => 0, 'all' => 0 ];
                    }
                ...
                {
                    $sale_channels[ 'managers' ][ $r[ 'sale_channel' ] ] =
                        [ 'sale_channel_id' => $r[ 'sale_channel_id' ], 'vpbx' => [ 'new' => 0, 'old' => 0, 'all' => 0 ], 'nums' => [ 'new' => 0, 'old' => 0, 'all' => 0 ], 'lines' => [ 'new' => 0, 'old' => 0, 'all' => 0 ], 'clients' => [ ], 'visits' => 0, 'courier_id' => $r[ 'courier_id' ] ];
                }
            ...
            foreach ( $res_vpbx as $r )
            {
                ...
                {
                    $sale_vpbx[ $r[ 'region' ] ] = [ 'new' => 0, 'old' => 0, 'all' => 0 ];
                }
                ...
                if ( ! isset( $clients_vpbx[ $r[ 'client_id' ] ] ) )
                {
                    ...
                    {
                        $vpbx_clients[ $r[ 'region' ] ] = [ 'new' => 0, 'old' => 0, 'all' => 0 ];
                    }
      ...
      foreach ($res as $r) {
        foreach ($sale_channels['managers'] as $manager=>&$val) {
            ...
            {
                if ( $sale_channels[ "all" ][ "nums" ][ 'all' ] >
                    0
                )
                {
                    $d[ "nums_perc" ][ 'new' ] =
                        round( $d[ "nums" ][ 'new' ] /
                               $sale_channels[ "all" ][ "nums" ][ 'all' ] *
                               100 );
                    $d[ "nums_perc" ][ 'old' ] =
                        round( $d[ "nums" ][ 'old' ] /
                               $sale_channels[ "all" ][ "nums" ][ 'all' ] *
                ...
                if ( $sale_channels[ "all" ][ "lines" ][ 'all' ] >
                    0
                )
                {
                    $d[ "lines_perc" ][ 'new' ] =
                        round( $d[ "lines" ][ 'new' ] /
                               $sale_channels[ "all" ][ "lines" ][ 'all' ] *
                               100 );
                    $d[ "lines_perc" ][ 'old' ] =
                        round( $d[ "lines" ][ 'old' ] /
                               $sale_channels[ "all" ][ "lines" ][ 'all' ] *
                ...
                if ( $sale_channels[ "all" ][ "vpbx" ][ 'all' ] >
                    0
                )
                {
                    $d[ "vpbx_perc" ][ 'new' ] =
                        round( $d[ "vpbx" ][ 'new' ] /
                               $sale_channels[ "all" ][ "vpbx" ][ 'all' ] *
                               100 );
                    $d[ "vpbx_perc" ][ 'old' ] =
                        round( $d[ "vpbx" ][ 'old' ] /
                               $sale_channels[ "all" ][ "vpbx" ][ 'all' ] *
        ...
        $design->assign( "RManager",
                         ( new \yii\web\View() )->renderFile( '@app/views/stats/report/_phone-sales-by-manager.php', [
                             ...
                             'managers'  => \app\classes\stats\PhoneSales::reportByManager(
                                 ...
                                 $to_m .
                                 '-' .
                                 date( 't', mktime( 0, 0, 0, $to_y, 1, $to_m ) ) // Делаем дату финальным днем конечного месяца
                             ),
                         ] ) );
...
*/
    ...
    function stats_report_by_one_manager()
    {
        ...
        $reportType =
            isset( $_POST[ 'type' ] ) ? $requestData->report .
                '_' .
                $requestData->type : $requestData->report;
        $managerReport =
            \app\classes\stats\PhoneSales::reportBySingleManager( $requestData->manager, $requestData->startDate, $reportType );
    ...
    function stats_report_vpbx_stat_space( $fixclient )
    {
        ...
        $options = [ ];
        $options[ 'select' ] =
            'C.id as client_id, UV.id, UV.client, UNIX_TIMESTAMP(LT.date_activation) as actual, T.description as tarif ';
        ...
        $options[ 'conditions' ] = [ $condition_string ];
    ...
    function getReportVpbxStatSpace( $client_id, $vpbx_id, $from, $to )
    {
        ...
        {
            $stat_detailed =
                VirtpbxStat::getVpbxStatDetails( $client_id, $vpbx_id, strtotime( $from ), strtotime( $to ) );
        }
...
class requestPlusOper
{
    function create( $client, $d )
    {
        ...
        $ai = [
            ...
            'ЛицевойСчет'             => '',
            'НомерПодключения'        => '',
            'Комментарий1'            => ( isset( $d[ "date_deliv" ] ) ? "Доставка на: " .
                $d[ "date_deliv" ] : "" ),
            'Комментарий2'            => ( isset( $d[ "fio_oper" ] ) ? "Оператор: " .
                $d[ "fio_oper" ] : "" ),
            'ПаспортСерия'            => '',
            'ПаспортНомер'            => '',
            'ПаспортКемВыдан'         => '',
        ...
        )
        {
            if ( $d[ "qty" ] )
            {
                $res[ "items_list" ][] =
                    [
                        "id"       => "4e8cc21b-d476-11e0-9255-d485644c7711" .
                            ":" .
                            $zeroDescr,
                        "quantity" => $d[ "qty" ],
                        "code_1c"  => 0,
                ...
                [
                    "id"       => ( $d[ "deliv_type" ] ==
                        "moskow" ?
                            ...
                            "1ccf5f37-03be-11e1-aefc-00155d881200" ) .
                        ":" .
                        $zeroDescr,
                    "quantity" => 1,
                    "code_1c"  => 0,
        ...
        else
        {
            $res[ "items_list" ][] =
                [
                    "id"       => "f75a5b2f-382f-11e0-9c3c-d485644c7711" .
                        ":" .
                        $zeroDescr,
                    "quantity" => $d[ "qty" ],
                    "code_1c"  => 0,
                ...
                [
                    "id"       => ( $d[ "deliv_type" ] ==
                        "moskow" ?
                            ...
                            "81d52245-4d6c-11e1-8572-00155d881200" ) .
                        ":" .
                        $zeroDescr,
                    "quantity" => 1,
                    "code_1c"  => 0,
