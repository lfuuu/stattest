...
class m_services extends IModule
{
    function GetMain($action, $fixclient)
    {
        ...
        call_user_func([$this, 'services_' . $action], $fixclient);
    }
    ...
    function services_default($fixclient)
    {
        ...
        if (access_action('services', 'vo_view')) {
            $this->services_vo_view($fixclient);
            return;
        }
        if (access_action('services', 'trunk_view')) {
            $this->services_trunk_view($fixclient);
            return;
        }
        if (access_action('services', 'in_view')) {
            $this->services_in_view($fixclient);
            return;
        }
        if (access_action('services', 'co_view')) {
...
// ==========================================================================================================================================
    function services_in_async($fixclient)
    {
        ...
        if ($port_type == "pppoe") {
            $port_type = 'pppoe","dedicated';
        }
        $R = [];
        $db->Query('select port_name from tech_ports where node="' . $node . '" and port_type IN ("' . $port_type . '") order by port_name');
        while ($r = $db->NextRecord()) {
            $R[$r['port_name']] = $r['port_name'];
        }
        $_RESULT = [
            'ports' => $R,
        ];
    ...
    function services_in_report()
    {
        ...
        $design->assign('port_types', TechPort::$portTypes);
        ...
        if (!$this->fetch_client($fixclient)) {
            ...
            foreach ($ports as $id_port => $port) {
                ...
                foreach ($connections[$id_port]['nets'] as $net) {
                    if ($net['actual']) {
                        $connections[$id_port]['route'] = $net['net'];
                    }
                }
    ...
    function services_in_apply($fixclient, $suffix = 'internet', $suffix2 = 'in')
    {
        ...
        $dbf->Display(['module' => 'services', 'action' => 'in_apply'], 'Услуги', 'Редактировать подключение');
    }
    ...
    function services_in_apply2($fixclient, $suffix = 'internet', $suffix2 = 'in')
    {
        ...
        if ($result == 'delete') {
            ...
            if (($result == 'add' || $result == 'edit') && !Yii::$app->session->get('error')) {
                Header('Location: /?module=services&action=in_view');
                exit;
            } else {
                $dbf->Display(['module' => 'services', 'action' => 'in_apply2'], 'Услуги', 'Редактировать сеть');
            }
        }
    }
    ...
    function services_in_act($fixclient, $suffix = "internet")
    {
        ...
        $routes = [];
        $db->Query('select * from usage_ip_routes where (port_id="' . $id . '") and (actual_from<=NOW()) and (actual_to>=NOW()) order by id');
        while ($r = $db->NextRecord()) {
            $t = explode("/", $r['net']);
            $t = explode('.', $t[0]);
            $t[3]++;
...
// =========================================================================================================================================
    function services_vo_view($fixclient)
    {
        ...
        switch ($sort = get_param_integer('sort', 3)) {
            case 3:
                ...
                break;
            case 4:
                $order = 'no_of_lines ' . $order;
                break;
            case 6:
                $order = 'client ' . $order;
                break;
            default:
                $order = 'actual_from ' . $order . ',actual_to ' . $order;
                break;
        }
    ...
    function services_vo_permit($fixclient)
    {
        ...
        if (!access("services_voip", "view_reg")) {
            return;
        }
    ...
    private function getSIPregs($cl, $region, $phone = "")
    {
        ...
        if ($region == 99) {
            ...
            if (in_array($region, [94, 95, 96, 87, 97, 98, 88, 89, 93, 991])) // new schema. scynced
            {
                $schema = "astschema";
        ...
        while ($l = pg_fetch_assoc($result)) {
            if (!isset($l["regvalid"])) {
                $l["regvalid"] = $l["regtime"] > 0;
            }

            if (isset($pbx[$l["ippbx"]])) {
                $l["ippbx"] = $pbx[$l["ippbx"]];
            }
            ...
            if (!isset($l["invite_contact"])) {
                ...
                $l["invite_contact"] = str_replace(["<", ">"], "", $l["invite_contact"]);
            }
    ...
    function services_vo_settings_send($fixclient)
    {
        ...
        if (get_param_raw("do_send", "") !== "") {
            $isSent = true;
            $_e164s = get_param_raw("e164", []);
            $_emails = get_param_raw("email", []);
            ...
            if (!$error) {
                foreach ($_emails as $_email) {
                    $db->QueryInsert("log_send_voip_settings", [
                            "client" => $clientNick,
                            "date" => ["NOW()"],
                            "user" => $user->Get("user"),
                            "email" => $_email,
    ...
    function services_vo_add($fixclient)
    {
        ...
        $dbf->Display(['module' => 'services', 'action' => 'vo_apply'], 'Услуги', 'Новое VoIP-подключение');
    }
    ...
    function services_vo_apply($fixclient)
    {
        ...
        $dbf->nodesign = true;
        $dbf->Display(['module' => 'services', 'action' => 'vo_apply'], 'Услуги', 'Редактировать VoIP-подключение');
...
// =========================================================================================================================================
    function services_dn_view($fixclient)
    {
        ...
        switch ($sort = get_param_integer('sort', 1)) {
            case 2:
                $order = 'primary_mx ' . $order;
                break;
            case 3:
                $order = 'paid_till ' . $order;
                break;
            case 4:
                $order = 'client ' . $order;
                break;
    ...
    function services_dn_add($fixclient)
    {
        ...
        $dbf->Display(['module' => 'services', 'action' => 'dn_apply'], 'Услуги', 'Новое доменное имя');
    }
    ...
    function services_dn_apply($fixclient)
    {
        ...
        $dbf->Display(['module' => 'services', 'action' => 'dn_apply'], 'Услуги', 'Редактировать доменное имя');
    }
...
// =========================================================================================================================================
    ...
    function whitelist_load($fixclient, $filter, &$domains, &$mails, &$MCN, &$whlist)
    {
        ...
        global $db, $design;
        $M = [/*0=>'Доставлять всё',*/
            1 => 'Добавлять в тему письма метку ---SPAM---',
            2 => 'Уничтожать'
        ];
        ...
        $domains = [];
        $db->Query('select domain from domains where (client="' . $clientNick . '") and (actual_from<=NOW()) and (actual_to>NOW())' . $filter_domain_q);
        while ($r = $db->NextRecord()) {
            $domains[] = $r['domain'];
        }
        ...
        while ($r = $db->NextRecord()) {
            $mails[$r['id']] = $r;
            if (($r['domain'] == "mcn.ru") && (!$filter_domain || ($filter_domain == "mcn.ru"))) {
                $MCN[] = $r['local_part'];
            }
        }
        ...
        if ($filter_domain_q) {
            $dq = "";
            if (count($domains) > 0) {
                $dq = '(domain IN ("' . implode('","', $domains) . '") AND ((local_part="") or (local_part="' . $filter_mail . '"))) OR ';
            }
        } else {
            $dq = "";
            if (count($domains) > 0) {
                $dq = '(domain IN ("' . implode('","', $domains) . '")) OR ';
            }
        }
        $mq = "";
        if (count($MCN) > 0) {
            $mq = '(domain="mcn.ru" and local_part IN ("' . implode('","', $MCN) . '")) OR ';
        }
    ...
    function services_em_whitelist_toggle($fixclient)
    {
        ...
        if ($r['spam_act'] == $mode) {
            return $this->services_em_view($fixclient);
        }
        if ($mode == 'pass') {
    ...
    function services_em_whitelist($fixclient)
    {
        ...
        if (!count($mails)) {
            trigger_error2('У вас нет ни одного почтового ящика');
            return;
        }
    ...
    function services_em_apply($fixclient)
    {
        ...
        if ($result == 'delete' || $result == 'add') {
            ...
            $dbf->Display(['module' => 'services', 'action' => 'em_apply'], 'Услуги', 'Редактировать e-mail ящик');
        }
    }
    ...
    function services_em_chreal($fixclient)
    {
        ...
        if ($r['client'] != $clientNick) {
            trigger_error2('Клиенты не совпадают');
            return;
        }
        if ($pass1 != $pass2) {
...
// =========================================================================================================================================
    ...
    function services_ex_add($fixclient)
    {
        ...
        $dbf->Display(['module' => 'services', 'action' => 'ex_apply'], 'Услуги', 'Новая дополнительная услуга');
    }
    ...
    function services_ex_apply($fixclient)
    {
        ...
        $dbf->Display(['module' => 'services', 'action' => 'ex_apply'], 'Услуги', 'Редактировать дополнительную услугу');
    }
...
// =========================================================================================================================================
    ...
    function services_it_add($fixclient)
    {
        ...
        $dbf->SetDefault('client', $clientNick);
        $dbf->Display(['module' => 'services', 'action' => 'ex_apply'], 'Услуги', 'Новая услуга ITPark');
    }
...
// =========================================================================================================================================
    ...
    function services_virtpbx_add($fixclient)
    {
        ...
        $dbf->SetDefault('client', $clientNick);
        $dbf->Display(['module' => 'services', 'action' => 'virtpbx_apply'], 'Услуги', 'Новая услуга Виртальная АТС');
    }
    ...
    function services_virtpbx_apply($fixclient)
    {
        ...
        $dbf->Display(['module' => 'services', 'action' => 'virtpbx_apply'], 'Услуги', 'Редактировать дополнительную услугу');
    }
    ...
    function services_virtpbx_act($fixclient)
    {
        ...
        if (!$id) {
            trigger_error2('Ошибка в данных');
            return;
        }
...
// =========================================================================================================================================
    ...
    function services_sms_add($fixclient)
    {
        ...
        $dbf->SetDefault('client', $clientNick);
        $dbf->Display(['module' => 'services', 'action' => 'sms_apply'], 'Услуги', 'Новая услуга CMC');
    }
    ...
    function services_sms_apply($fixclient)
    {
        ...
        $dbf->Display(['module' => 'services', 'action' => 'sms_apply'], 'Услуги', 'Редактировать услугу CMC');
    }
    ...
    function services_welltime_add($fixclient)
    {
        ...
        $dbf->Display(['module' => 'services', 'action' => 'welltime_apply'], 'Услуги', 'Новая услуга Welltime');
    }
    ...
    function services_welltime_apply($fixclient)
    {
        ...
        $dbf->Display(['module' => 'services', 'action' => 'welltime_apply'], 'Услуги', 'Редактировать дополнительную услугу');
    }
...
// =========================================================================================================================================
    ...
    function services_wellsystem_add($fixclient)
    {
        ...
        $dbf->SetDefault('client', $clientNick);
        $dbf->Display(['module' => 'services', 'action' => 'ex_apply'], 'Услуги', 'Новая услуга WellSystem');
    }
...
// =========================================================================================================================================
    ...
    function services_ppp_add($fixclient)
    {
        ...
        $dbf = new DbFormUsageExtra();
        $dbf->SetDefault('client', ClientAccount::findOne($fixclient)->client);
        $dbf->Display(['module' => 'services', 'action' => 'ppp_apply'], 'Услуги', 'Новый ppp-логин');
    }
    ...
    function services_ppp_apply($fixclient)
    {
        ...
        $dbf->Display(['module' => 'services', 'action' => 'ppp_apply'], 'Услуги', 'Редактировать ppp-логин');
    }
    ...
    function services_ppp_activateall($fixclient)
    {
        ...
        $clientNick = ClientAccount::findOne(['id' => $fixclient])->client;
        $value = get_param_integer('value', 0);
        if ($value) {
            $value = 1;
        }
        if ($value == 0) {
...
// =========================================================================================================================================
    function &Tarif($id)
    {
        ...
        $db->Query('select * from tarifs_internet where id=' . $id);
        $this->T[$id] = $db->NextRecord();
        return $this->T[$id];
    }
...
//            $r['tarif_next']=get_tarif_next("usage_ip_ports",$r['id']);
    ...
    function routes_check($client)
    {
        ...
        if (isset($r[0]) && $r[0] == "0") {
            ...
            $r = $db->NextRecord();
            if (isset($r[0]) && ($r[0])) {
                trigger_error2("Внимание! У клиента осталась IP-телефония.");
            }
        } else {
    ...
    function services_rpc_check_e164()
    {
        ...
        global $db;
        $spec_numbers = ['9999', '7495', '7499', '', '0', '101'];
        ...
        if (preg_match('/^FREE:(\d{2,7}|short)?/', $_GET['e164'], $m)) {
            if (isset($m[1]) && $m[1] != 'short') {
                ...
                if (($res = $db->GetValue($query)) !== false) {
                    while (in_array($res, $spec_numbers)) {
                        $res++;
                    }
                    return $res;
                } else {
...
class voipRegion
{
    ...
    static private function getRegionRegs($client, $region, $_e164s)
    {
        ...
        while ($l = pg_fetch_assoc($result)) {
            if (isset($pbx[$l["ippbx"]])) {
                ...
                die("Неизветсный IPPBX, обратитесь к программисту");
            }
    ...
    {
        if (self::$lastRegionServer == $region) {
            return;
        }
        ...
        if ($region == "99") {
            ...
            if (in_array($region, [94, 95, 87, 97, 98, 88, 89, 93, 991])) // new schema. scynced
            {
                $schema = "astschema";
                $dbHost = "eridanus.mcn.ru";
            }
