...
//                $R[$k]['sms'] = $db->GetRow("select * from newbill_sms where bill_no = '".$r["bill_no"]."' limit 1");
    ...
    function    showTroubleList($mode,$tt_design = 'full',$fixclient = null,$service = null,$service_id = null,$t_id = null, $server_ids = null){
        ...
        if($this->dont_filters || $tt_design != "full")// || isset($_REQUEST['filters_flag']))
            ...
            $design->assign('users',\app\models\UserGroups::dao()->getListWithUsers());
        }
                ...
                FROM
                    (
                        ...
                        INNER JOIN ( 
                            ...
                            where model='app\\\\models\\\\Bill' and data_json like '%" . ($doerId ? "\"courier_id\":\"{$doerId}\"":"\"courier_id\":") ."%'
                            group by model_id
                    ...
                    (
                        ...
                        AND
                            `ln`.`id` = (
                                    select id
                                    from `history_changes`
                                    where model='app\\\\models\\\\Bill' and model_id=nb.id and data_json like concat('%\"courier_id\":\"',cr.id,'\"%')
