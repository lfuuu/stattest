...
class DbForm {
    ...
    public function Process($no_real_update = 0){
        ...
        if($this->dbform_action=='delete'){
            ...
            if (!$no_real_update) {
                $p = $usageForm
                    ? $this->processUsageForms($usageForm, $s, $sDiff)
                    : $this->processOtherForms($s, $sDiff);
            }
            else {
                $p = 'add';
    ...
    /**
     * @param \app\classes\Form $usageForm
     * @param array $fields
     * @param string $sDiff
     * @return string
     */
    private function processUsageForms(\app\classes\Form $usageForm, array $fields, $sDiff)
    {
        $p = null;
        ...
        if ($this->dbform['id']) {
            ...
            if (
                $usageForm->load([$usageForm->formName() => $fields])
                &&
                $usageForm->validate()
                &&
                $usageForm->saveModel($model, true, true)
     ...
     * @return string
     */
    private function processOtherForms(array $fields, $sDiff)
    {
        global $db;
        $p = null;
        ...
        if ($this->dbform['id']) {
            ...
            $db->Query($q='update '.$this->table.' SET '.implode(',', $fields).' WHERE id='.$this->dbform['id']);
            $p='edit';
            Yii::$app->session->addFlash('success', 'Запись обновлена');
        } else {
            $db->Query('insert into '.$this->table.' SET '.implode(',', $fields));
...
class DbFormUsageVoip extends DbForm {
    ...
    private function makeFormData()
    {
        ...
        $data = [
            ...
            "multis" => app\classes\api\ApiPhone::me()->getMultitranks()
            ];
...
$GLOBALS['translate_arr']=array(
    ...
    '*.port_type'            => 'тип порта',
    '*.tarif'                => 'тариф',
    '*.trafcounttype'        => 'тип учёта трафика',
    ...
    'usage_tech_cpe.admin_login'    => 'Логин администратора',
    'usage_tech_cpe.admin_pass'    => 'Пароль администратора',
    ...
    '*.month_r'        => 'Мб трафика Russia в месяц',
    '*.month_r2'    => 'Мб трафика Russia2 в месяц',
    '*.month_f'        => 'Мб трафиа Foreign в месяц',
