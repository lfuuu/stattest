...
class citiPaymentsInfoParser
{
    function parse($f)
    {
        ...
        {
            $d = array();
            preg_match_all("/Сумма[^\d]+(?P<sum>\d+-\d+) (?P<company>.*)/", $b, $o);
            $d["sum"] = (float)str_replace("-", ".", $o["sum"][0]);
            $d["company"] = trim($o["company"][0]);
            preg_match_all("/Cч.[^\d]+(?P<account>[\d]+)/", $b, $o);
            if($o["account"][0] == 30101810300000000202) $idx = 1; else $idx =0;

            preg_match_all("/ИНН (?P<inn>[\d]+)/", $b, $o);
            $d["inn"] = trim($o["inn"][$idx]);

            preg_match_all("/Cч.[^\d]+(?P<account>[\d]+)/", $b, $o);
            $d["account"] = $o["account"][$idx];

            preg_match_all("/ПЛАТЕЖНОЕ ПОРУЧЕНИЕ[^\d]+(?P<payment_no>\d+)/", $b, $o);
