...
class DbViewCommonTarif extends DbView
{
    public function __construct()
    {
        ...
        $this->SQLFilters['t'] = 'status="test"';
        $this->SQLFilterNames['p'] = 'публичный';
        $this->SQLFilterNames['a'] = 'архивный';
        $this->SQLFilterNames['s'] = 'специальный';
        $this->SQLFilterNames['t'] = 'тестовый';

        $currencies = Currency::enum();

        foreach ($currencies as $currency) {
            $this->SQLFilters[$currency] = 'currency="' . $currency . '"';
            $this->SQLFilterNames[$currency] = Currency::symbol($currency);
        }
...
class DbFormTarifsInternet extends DbFormSimpleLog
{
    public function constructChild()
    {
        ...
        $this->fields['type_internet'] = array(
            'assoc_enum' => array(
                'standard' => 'Обычный',
                'wimax' => 'WiMAX',
        ...
        $this->fields['type_count'] = array(
            'assoc_enum' => array(
                'all' => 'всё раздельно',
                'r2_f' => 'Считать Россию-2 как иностранный',
                'all_f' => 'Считать всё как иностранный'
            )
        );
...
class DbViewTarifsExtra extends DbViewCommonTarif
{
    public function constructChild()
    {
        ...
        $this->SQLFilterGroups['Группа тарифов'] = [
            'allcode',
            'nocode',
            'domain',
            'ip',
...
class DbFormTarifsExtra extends DbFormSimpleLog
{
    public function constructChild()
    {
        ...
        $this->fields['period'] = array(
            'assoc_enum' => array(
                'month' => 'ежемесячно',
                'year' => 'ежегодно',
                'once' => 'разово',
                '3mon' => 'раз в 3 месяца',
                '6mon' => 'раз в 6 месяцев'
            )
        );
        $this->fields['price_include_vat'] = array('type' => 'checkbox');
    }
...
class DbViewTarifsVirtpbx extends DbView
{
    public function __construct()
    {
        ...
        $this->SQLFilterGroups['Состояние'] = ['public', 'special', 'archive', 'test'];
        $this->SQLFilterGroups['Валюта тарифа'] = Currency::enum();
        ...
        foreach (Currency::enum() as $currencyKey) {
            $this->SQLFilters[$currencyKey] = "currency = '" . $currencyKey . "'";
        }
...
class DbFormTarifsSms extends DbFormSimpleLog
{
    public function constructChild()
    {
        ...
        $this->fields['currency'] = array('type' => 'hidden', 'default' => 'RUB');
        $this->fields['status'] = array('assoc_enum' => array('public' => 'публичный', 'archive' => 'архивный'));
        $this->fields['description'] = array();
...
class DbViewFactory
{
    ...
    {
        if ($v == 'internet') {
            return new DbViewTarifsInternet();
        }
        if ($v == 'extra') {
            return new DbViewTarifsExtra();
        }
        if ($v == 'itpark') {
            return new DbViewTarifsITPark();
        }
        if ($v == 'welltime') {
            return new DbViewTarifsWelltime();
        }
        if ($v == 'virtpbx') {
            return new DbViewTarifsVirtpbx();
        }
        if ($v == 'wellsystem') {
            return new DbViewTarifsWellSystem();
        }
        if ($v == 'sms') {
            return new DbViewTarifsSms();
        }
        return false;
    }
    ...
    {
        if ($v == 'internet' && $t == 'c') {
            return new DbFormTarifsCollocation();
        }
        if ($v == 'internet') {
            return new DbFormTarifsInternet();
        }
        if ($v == 'extra') {
            return new DbFormTarifsExtra();
        }
        if ($v == 'itpark') {
            return new DbFormTarifsITPark();
        }
        if ($v == 'welltime') {
            return new DbFormTarifsWelltime();
        }
        if ($v == 'virtpbx') {
            return new DbFormTarifsVirtpbx();
        }
        if ($v == 'wellsystem') {
            return new DbFormTarifsWellSystem();
        }
        if ($v == 'sms') {
            return new DbFormTarifsSms();
        }
        return false;
    }
}
