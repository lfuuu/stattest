...
class billMaker{
    ...
    public function getStatOrder($bill_no,&$fault=null)
    {
        ...
        if(!$bill)
        {
            trigger_error2("getStatOrder: счет не найден:". $bill_no);
            return false;
        }
        ...
        try{
            ...
            trigger_error2(trr($e->getMessage()));
            return false;
        }
         */
...
#1c-vitrina
    ...
    public function statSetOrderStatus($data)
    {
        ...
        return array('return'=>!mysqli_errno());
    }
    ...
    public function statSavePriceType($data)
    {
        ...
        $err = mysqli_errno();
        if($err) {
            return new \SoapFault('statSavePriceType',tr('ошибка создания типа цены: '.mysqli_error()));
        }
        return array('return'=>true);
    }
    ...
    public function statSaveUnit($data)
    {
        ...
        $err = mysqli_errno();
        if($err) {
            return new \SoapFault('statSaveUnit',tr('ошибка создания Единица Измерения: '.mysqli_error()));
        }
        return array('return'=>true);
    }
    ...
    public function statSaveStoreBalance($data)
    {
        ...
        if(mysqli_errno()) {
            return new \SoapFault('statSaveStoreBalance', mysqli_error());
        }
    ...
    public function statSaveGood($data)
    {
        ...
        if(isset($data->{tr("Товар")}->{tr("СписокХарактеристик")})) {
            ...
            foreach($hs as $h){
                ...
                if($err)
                {
                    return new \SoapFault('goods',tr('ошибка создания характеристики товара: '.mysqli_error()));
                }
            }
        }
