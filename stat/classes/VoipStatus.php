...
class VoipStatus {
    ...
    public function loadVoipCounters(){
        ...
        try {
            $counters_reg =
                $pg_db->GetRow("SELECT   CAST(amount_sum as NUMERIC(8,2)) as amount_sum, CAST(amount_day_sum as NUMERIC(8,2)) as amount_day_sum, CAST(amount_month_sum as NUMERIC(8,2)) as amount_month_sum
                                           FROM billing.counters
                                           WHERE client_id='".$this->counters["id"]."'");
            $counters_locks =
                $pg_db->GetRow("SELECT   voip_auto_disabled, voip_auto_disabled_local
                                           FROM billing.locks
                                           WHERE client_id='".$this->counters["id"]."'");
