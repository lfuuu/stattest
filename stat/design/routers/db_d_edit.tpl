...
<TBODY>
<TR><TD class=left>id:</TD><TD>{$id}</td></Tr>
<TR><TD class=left>client:</TD><TD>{$row.client.value}</td></Tr>
{foreach from=$row item=item key=key name=outer}
{dbmap_element}
{/foreach}

</TBODY></TABLE>
{if access('routers_devices','edit')}<DIV align=center><INPUT id=submit class=button type=submit value="Изменить"></DIV>{/if}
</form>