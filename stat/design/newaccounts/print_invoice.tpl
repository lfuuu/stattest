...
.hst {width: 100%}
.hst td {font-size: 9pt;}
.hst .ff {width: 5%; padding-right: 1px;white-space: nowrap; padding-left: 130px;}
.fs {font-size: 11pt !important;}
.hst .f {width: 5%; padding-right: 1px;white-space: nowrap;}
.hst .n {border-bottom: 1px solid black;width: 94%;}
.hst .l {width: 10px;}
{/literal}
{/if}
</style>
...
{else}
  <td valign="top" width="55%" class="ht">
          {if !$inv_is_new7}
    Продавец: <strong>{if '2009-06-01' < $bill.bill_date || ($bill.bill_date eq '2009-06-01' && $invoice_source != 2)}{$firm.name_full}{if $bill.bill_date < '2012-01-24'} ({$firm.name}){/if}{else}{$firm.name}{/if}</strong><br>
    Адрес: <strong>{$firm.address}</strong><br>
    ИНН/КПП продавца: <strong>{$firm.inn} / {$firm.kpp}</strong><br>
...
К платежно-расчетному документу{if isset($inv_pays)} {foreach from=$inv_pays item=inv_pay name=outer}N{$inv_pay.payment_no} от {$inv_pay.payment_date_ts|mdate:"d.m.Y г."}{if !$smarty.foreach.outer.last}, {/if}{/foreach}{/if}<br>
    ...
    {*'2017-07-01' = 1498867200*}{if 1498867200 <= $inv_date}<br>Идентификатор государственного контракта, договора (соглашения){if $isChanges20171001} (при наличии){/if}:
    <br/><br/>{/if}
          {/if}
          ...
          <table border="0" cellpadding="0" cellspacing="0" class="hst">
              <tr>
                  ...
                  <td class="n">{if (('2009-06-01' < $bill.bill_date || ($bill.bill_date eq '2009-06-01' && $invoice_source != 2)) && $invoice_source != 3) || $is_four_order}{section loop="40" name="mysec"}&nbsp;{/section}------{else}<strong>{$firm.name}</strong><strong>{$firm.address}</strong>{/if}</td>
                  <td class="l">(3)</td>
              </tr>
          </table>
          ...
          <table border="0" cellpadding="0" cellspacing="0" class="hst">
              <tr>
                  ...
                  <td class="n">{if isset($bill_client.is_with_consignee) && $bill_client.is_with_consignee && $bill_client.consignee}<strong>{$bill_client.consignee}</strong><br>{else}{if (('2009-06-01' < $bill.bill_date || ($bill.bill_date eq '2009-06-01' && $invoice_source != 2)) && $invoice_source != 3) || $is_four_order}{section loop="42" name="mysec"}&nbsp;{/section}------{else}<strong>{$bill_client.company_full}</strong><strong>{$bill_client.address_post}</strong>{/if}{/if}</td>
                  <td class="l">(4)</td>
              </tr>
          </table>
          ...
          <table border="0" cellpadding="0" cellspacing="0" class="hst">
              <tr>
                  ...
                  <td class="n"><strong>{if $bill_client.head_company}{$bill_client.head_company}{else}{$bill_client.company_full}{/if}</strong></td>
                  <td class="l">(6)</td>
              </tr>
          </table>
          ...
          <table border="0" cellpadding="0" cellspacing="0" class="hst">
              <tr>
                  <td class="f">Адрес: </td>
                  <td class="n"><strong>{if $bill_client.head_company_address_jur}{$bill_client.head_company_address_jur}{else}{$bill_client.address_jur}{/if}</strong></td>
                  <td class="l">(6а)</td>
              </tr>
          </table>
          ...
          <table border="0" cellpadding="0" cellspacing="0" class="hst">
              <tr>
                  <td class="f">Идентификатор государственного контракта, договора (соглашения){if $isChanges20171001} (при наличии){/if}:</td>
                  <td class="n">{section loop="43" name="mysec"}&nbsp;{/section}----</td>
                  <td class="l">(8)</td>
              </tr>
          </table>
  ...
  <tr>
    ...
    <td colspan="2">
    {if !$inv_is_new3}<p style="text-align:center;"><strong>СЧЕТ-ФАКТУРА N&nbsp;{if !$inv_number}{if $is_four_order eq true}AB-{/if}{$bill.bill_no}{$inv_no}{else}{$inv_number}{/if}
        {if !$without_date_date}
            от  {if $is_four_order && isset($inv_pays)}
                    {$inv_pays[0].payment_date_ts|mdate:"d.m.Y г."}
                {else}
...
{*if !$inv_is_new3}Наименование и код валюты: руб. (643){/if*}
    <div style="text-align:center;"><center><table border="1" cellpadding="3" cellspacing="0" width="100%">
      <tr>
      ...
      <tr>
        ...
        {if $inv_is_new3}<td style="text-align:center;">2а</td>{/if}
        ...
        {if $inv_is_new3}<td style="text-align:center;">10а</td>{/if}
...
{foreach from=$bill_lines item=row key=key name=lines}
      <tr>
          {if $inv_is_new7}<td style="text-align:center;">{$smarty.foreach.lines.iteration}</td>{/if}
...
{/if}
      ...
      <td style="text-align:center;">
        ...
        {if $inv_is_new3}<td style="text-align:center;">{if !isset($row.country_id) || $row.country_id == 0}-{else}{$row.country_id}{/if}</td>{/if}
...
{/if}
    ...
    <table border="0" cellpadding="0" cellspacing="5" align="left">
      <tr >
        <td><p style="text-align:right;">Руководитель&nbsp;организации{if $inv_is_new4}<br>или иное уполномоченное лицо{/if}:</td>
        <td>
    ...
    <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
        <td><p style="text-align:right;">&nbsp;Главный&nbsp;бухгалтер{if $inv_is_new4}<br>или иное уполномоченное лицо{/if}:</td>
...
{/if}{*if $inv_is_new5*}
      <tr>
        <td colspan=4><p><br>Индивидуальный предприниматель{if $isChanges20171001}<br>или иное уполномоченное лицо{/if} ___________________&nbsp;&nbsp;&nbsp;&nbsp; _______________________</p>
    </td>
