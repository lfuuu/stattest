...
<option value='form.row_price.value="{$item.price}"; form.row_description.value="{$item.description}"; form.row2_period.value="{$item.period}"; {if $item.period=="once"}form.row_actual_to.value=form.row_actual_from.value;{else}form.row_actual_to.value="4000-01-01";{/if}'>{$item.description} | {$item.price}</option>
{/foreach}
</SELECT></TD></TR>
</TBODY></TABLE>
