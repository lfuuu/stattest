<H3>Создайте отчёт сами: (или - посмотрите отчёты за
    <a href="?module=stats&route={$route}&action=internet&is_coll={$is_collocation}&detality=day&date_from={$prev_date_from}&date_to={$prev_date_to}">прошлый
        месяц</a>,
    за
    <a href="?module=stats&is_coll={$is_collocation}&route={$route}&action=internet&detality=day&date_from={$cur_date_from}&date_to={$cur_date_to}">текущий
        месяц</a>,
    за
    <a href="?module=stats&is_coll={$is_collocation}&route={$route}&action=internet&detality=hour&date_from={$today}&date_to={$today}">текущий
        день</a>)</H3>
            ...
            <FORM action="?" method=get>
                ...
                <SELECT name=route>
                    ...
                    {foreach from=$routes_all item=item}
                        <option value="{$item.net}"{if $route==$item.net} selected{/if}>{$item.net} ({$item.actual_from}
                        - {$item.actual_from})</option>{/foreach}
                </SELECT><BR>
        </TD>
