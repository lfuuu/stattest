...
{foreach from=$report_data item="one_day" key="date"}
	<tr align='center'>
		...
		{foreach from=$one_day.doers item="day" key="doer"}
			...
			<td>{if !isset($print) || !$print}<a href='/clients/view?id={$day.client_id}' target='_blank' style='text-decoration:none;'>{$day.company}</a><br><br>{if $day.bill_no}<a href="./?module=newaccounts&action=bill_view&bill={$day.bill_no}" target=_blank>{$day.bill_no}</a>{if $view_calc}<br><br>
