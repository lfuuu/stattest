...
<table class="table table-condensed table-striped">
    {if !$tt_trouble.bill_no}
        {if $tt_client}
            <tr>
                ...
                <td>
                    <div class="row">
                        <div class="col-sm-6">
                            <a href='/client/view?id={$tt_client.id}'>{$tt_client.name}</a> ({$tt_client.client})
                        </div>
                        ...
                        <div class="col-sm-6">
                            {if $is_trouble_with_lead}
                                ...
                                <div class="input-group col-sm-8">
                                    <input type="text" class="form-control account-search ui-autocomplete-input"
                    ...
                    <script>
                      optools.moveTroubleToClient.init({$tt_trouble.id})
                    </script>
                </td>
            </tr>
        {/if}
        {if $tt_trouble.service}
            <tr>
                ...
                <td>
                    <a href='pop_services.php?table={$tt_trouble.service}&id={$tt_trouble.service_id}'>{if $tt_trouble.service=="usage_voip"}Телефония {$tt_trouble.number}{else}{$tt_trouble.service} #{$tt_trouble.service_id}{/if}</a>
                </td>
            </tr>
        {/if}
        {if $tt_trouble.server_id}
            <tr>
                ...
                <td><a href='module=routers&action=server_pbx_apply&id={$tt_trouble.server_id}'>{$tt_trouble.server},
                        Тех.площадка: "{$tt_trouble.datacenter_name}", Регион: {$tt_trouble.datacenter_region}</a></td>
            </tr>
        {/if}
        {if $tt_trouble.bill_no}
            <tr>
                ...
                <td>
                    <a href="index.php?module=newaccounts&action=bill_view&bill={$tt_trouble.bill_no}">{$tt_trouble.bill_no}</a>
                </td>
            </tr>
        {/if}
        <tr>
    ...
    {if access('tt','time') && $tt_write && $tt_edit}
        <tr>
            ...
            <td>
                <form action='?' style='padding:0; margin:0' method=post><input type=hidden name=module value=tt><input
                            type=hidden name=action value=time><input type=hidden name=id value={$tt_trouble.id}><input
                            type=text class=text name=time value='1'> <input type=submit class=button value='Добавить'>
                    (введите отрицательное число, чтобы отнять время)
                </form>
            </td>
        </tr>
    {/if}
    {if access('tt','time') && $tt_write && $tt_edit}
        <tr>
            ...
            <td>
                <form action='?' style='padding:0; margin:0' method=post><input type=hidden name=module value=tt><input
                            type=hidden name=action value=time><input type=hidden name=id value={$tt_trouble.id}><input
                            type=text name=date_activation
                            value="{$tt_trouble.date_start|udate_with_timezone:'Y-m-d H:i:s':false}"> <input type=submit
                                                                                                             class=button
                                                                                                             value='Установить'>
                </form>
            </td>
            ...
            <table class="table table-condensed table-striped table-hover">
                ...
                {foreach from=$tt_trouble.stages item=item name=outer}
                    <tr>
                        ...
                        <td>{$item.date_start|udate_with_timezone:'m-d H:i'}
                            <br>{$item.date_finish_desired|udate_with_timezone:'m-d H:i'}</td>
                        <td>
                                ...
                                <tr>
                                    <td width="50%">&nbsp;
                                        ...
                                        {if isset($item.stages_comment) && $item.stages_comment}
                                            <br/>
                                            <span style="font-size: 7pt;">{$item.stages_comment}</span>
                                        {/if}
                                        {if count($item.doers)>0}
                                    </td>
                                    <td width="50%">
                            ...
                            {if isset($item.doer_stages) && $item.doer_stages}
                                <table border="0">
                                    {foreach from=$item.doer_stages item=ds}
                                        <tr>
                                            <td>{$ds.date}</td>
                                            <td>{$ds.status_text}({$ds.status})</td>
                                            <td>{$ds.comment}</td>
                                        </tr>
                                    {/foreach}
                                </table>
            ...
            {if count($tt_media)}
                ...
                <div class="media-list">
                    {foreach from=$tt_media item=file}
                        <div data-model="troubles" data-file-id="{$file.id}"
                             data-mime-type="{$file.mimeType}">{$file.name}</div>
                    {/foreach}
                </div>
                                        ...
                                        {foreach from=$tt_users item=item}
                                            {if $tt_trouble.user_main==$item.user}
                                                <input type=hidden name=user
                                                       value={$item.user}>{$item.name} ({$item.user})
                                            {/if}
                                        {/foreach}
                                    {else}
                                        ...
                                        {if access('tt','rating')} onChangeSelectState(this); {/if}
                                            ...
                                            {foreach from=$tt_states item=item}
                                                {if !isset($tt_restrict_states) || !($item.pk & $tt_restrict_states)}
                                                    <option id='state_{$item.id}' data-id="{$item.id}"
                                                            value='{$item.id}'{if $tt_trouble.state_id==$item.id} selected{/if}>{$item.name}</option>
                                                {/if}
                                            {/foreach}
                                        </select>
                                        {if isset($admin_order) && $admin_order}
                                            <input type=submit value="Предать в admin.markomnet" name="to_admin"
                                                   class=button>
                                        {/if}
                                    {/if}
...
{else}
                            ...
                            {if !(isset($admin_order) && $admin_order) || $order_editor == "stat"}
                                <tr>
                                    ...
                                    <td>
                                        <input type="checkbox"
                                               name="showTimeTable"{if isset($timetableShow)} checked='checked'{/if}
