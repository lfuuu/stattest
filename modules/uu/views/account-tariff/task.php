<script>
    function render(d) {
        ...
        $('#task_header').html(
            '<div class="progress">\n' +
            '  <div class="progress-bar progress-bar-' + bgClass + '" role="progressbar" style="width: ' + perc + '%" aria-valuenow="' + d.count_done + '" aria-valuemin="0" aria-valuemax="' + d.count_all + '">' + (d.status != 'plan' ? '<b>' + perc + '% </b><span style=\'padding-left: 50px;\'>' + d.count_all + ' / ' + d.count_done + '</span>' : 'Запланировано') + '</div>\n' +
            '</div>');
