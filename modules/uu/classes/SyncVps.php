...
class SyncVps
{
    ...
    public function syncVm($accountTariffId)
    {
        ...
        if ($accountTariff->vm_elid_id) {
            ...
            $apiVps->vpsUpdate(
                ...
                $resourceRam = (int)$tariffResources[ResourceModel::ID_VPS_RAM],
                $resourceProcessor = (int)$tariffResources[ResourceModel::ID_VPS_PROCESSOR]
            );
    ...
    public function syncResource($clientAccountId, $accountTariffId, $accountTariffResourceIds)
    {
        ...
        ApiVps::me()->vpsUpdate(
            $clientAccountId,
            $accountTariff->vm_elid_id,
            isset($resources[ResourceModel::ID_VPS_RAM]) ? $resources[ResourceModel::ID_VPS_RAM] : null,
            isset($resources[ResourceModel::ID_VPS_PROCESSOR]) ? $resources[ResourceModel::ID_VPS_PROCESSOR] : null,
            isset($resources[ResourceModel::ID_VPS_HDD]) ? $resources[ResourceModel::ID_VPS_HDD] : null
        );

        if (isset($resources[ResourceModel::ID_VPS_HDD])) {
            Trouble::dao()->createTrouble(
                $clientAccountId,
                Trouble::TYPE_TASK,
                Trouble::SUBTYPE_TASK,
                sprintf('Для VPS ELID ID = %d изменить ресурс HDD на %d GB. УУ %s', $accountTariff->vm_elid_id, $resources[ResourceModel::ID_VPS_HDD], $accountTariff->getUrl()),
                null,
