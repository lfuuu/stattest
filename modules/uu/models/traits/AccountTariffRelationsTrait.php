 ...
 * @property-read ResourceClass[] $resources
...
trait AccountTariffRelationsTrait
{
    ...
    {
        return $this->hasMany(ResourceClass::class, ['service_type_id' => 'service_type_id'])
            ->indexBy('id')
            ->orderBy(['id' => SORT_ASC]);
    }
    ...
    public function getAccountLogResourceOptions()
    {
        return $this->hasMany(AccountLogResource::class, ['account_tariff_id' => 'id'])
            ...
            ->where([
                'NOT', [TariffResource::tableName() . '.resource_id' => ResourceClass::getReaderIds()], // только опции
            ]);
    }
    ...
    public function getAccountLogResourceTraffics()
    {
        return $this->hasMany(AccountLogResource::class, ['account_tariff_id' => 'id'])
            ->joinWith('tariffResource')
            ->where([
                TariffResource::tableName() . '.resource_id' => ResourceClass::getReaderIds(), // только трафик
            ]);
    }
