...
trait AccountTariffBoolTrait
{
     ...
     * @return bool
     */
    public function isResourceCancelable(ResourceClass $resource)
    {
        if (!$this->isEditable()) {
     ...
     * @return bool
     */
    public function isResourceEditable(ResourceClass $resource)
    {
        if (!$this->isEditable()) {
            // услуга нередактируемая
            return false;
        ...
        if ($resource->id == ResourceClass::ID_VOIP_FMC && ($number = $this->number) && !$number->isFmcEditable()) {
            // Костыль для FMC. Редактируемость этого ресурса зависит от типа телефонного номера
            return false;
        }

        if ($resource->id == ResourceClass::ID_VOIP_MOBILE_OUTBOUND && ($number = $this->number) && !$number->isMobileOutboundEditable()) {
