    ...
    $report = array();
      ...
      $report =   $pg_db->AllRecords("
        ...
        foreach($r_pricelists as $pl)
        {
          if ($i == 0)
          {
            $r_amount = $r_prices[$i]*$r['volume'];
            $r_parts[$pl] = array('price'=>$r_prices[$i],'volume'=>$r['volume'] ? round($r['volume']) : '','amount'=>$r_amount ? round($r_amount) : '');
            if (!isset($totals[$pl])) $totals[$pl] = array('volume'=>'','amount'=>'','amount_op'=>'');

            if ($r['volume'])
            {
              $totals[$pl]['volume'] += $r['volume'];
              $totals['all']['volume'] += $r['volume'];
            }
            if ($r_amount)
            {
