...
class _voipnew_prices_parser{
	...
	public static function &read_table(&$objWorksheet, $fields){
		...
		foreach ($rowIterator as $row) {
			if(!$isFindHeader) {
				...
				if($isFindHeader){
					foreach ($fields as $k=>$f){
						if ($f['col'] === false) { $table = false; return $table; }
					}
				}
 			}
			else {	
	...
	public static function &read_beeline_full2($filename){
		...
		foreach ($table as $row){
			...
			foreach ($codes_list as $code){
				...
				if (count($period)==1){
					$n = trim($period[0]);
					if (!preg_match('#^\d*$#', $n)) { $table = false; return $table; }
					...
					$n2 = trim($period[1]);
					if (!preg_match('#^\d+$#', $n)) { $table = false; return $table; }
					if (!preg_match('#^\d+$#', $n2)) { $table = false; return $table; }
                    $l=strlen($n);
					$n = (int)$n;
					$n2 = (int)$n2;
					while ($n <= $n2){
	...
	public static function &read_beeline_changes($filename){
		...
		foreach ($table as $row){
			...
			if (count($del_list) < 2) continue; $del_list = trim($del_list[1]);
...
class m_voipnew extends IModule{
	...
	public function __call($method, array $arguments = array()) {
		foreach ($this->_inheritances as $inheritance) { $inheritance->invoke($method, $arguments); }
	}
	...
	public function voipnew_view_raw_file(){
		...
		$pg_db->Query('BEGIN');
		try{
			$query = "SELECT d.defcode, r.deleting, r.price, 
			...
			if ($page > 1){	$pg_db->Query('MOVE FORWARD '.(($page-1)*$recPerPage).' IN curs');
			...
			foreach ($defs as $row) {
				...
				if (count($new_rows) >= 10000){
					if (!$this->insert_raw_prices($new_rows)){
						echo $pg_db->mError;
						$pg_db->Rollback();
						$design->AddMain('voipnew/upload.html'); return 0;
					}
					$new_rows = array();
				}
			}
	...
	public function voipnew_upload()
	{
		...
		if (isset($_POST['step']) && $_POST['step']=='upfile'){
			if(!$_FILES['upfile']){
				...
				$design->AddMain('voipnew/upload.html'); return;
...
//			$design->assign('rows_count', count($defs));
			//header("location: index.php?module=voipnew&action=compare_raw_file&id=".intval($rawfile_id));
//			$design->AddMain('voipnew/compare_raw_file.html'); return;
		...
		if ($pricelist_id != ''){
            ...
            $destination = $r['destination'];
				...
				foreach ($resgroups as $resgroup)
				{
					...
					if ($defs != ''){ $resgroup['defcode'] = $resgroup['defcode'].' </b>' . '('.$defs.')<b>'; }
					$res[] = $resgroup;
				}
