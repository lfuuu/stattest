...
class m_voipnew_analyze_pricelist_report {
	...
	function load_params($report_id){
		...
		foreach($params as $p){
			...
			if ($p['param'] == 'd1') {	$m = explode('-', $p['date']);
										$param2['date1'] = $m[2].'.'.$m[1].'.'.$m[0];	} 
			if ($p['param'] == 'd2') {	$m = explode('-', $p['date']);
    ...
    $m = explode('-', $rep['volumes_date_from']); if (count($m)==3) $rep['volumes_date_from'] = $m[2].'.'.$m[1].'.'.$m[0];
	...
	function voipnew_analyze_pricelist_report_edit(){
		...
		if (isset($_POST['data'])){
			...
			foreach($selprices as $sp){
				...
				if ($sp['date1'] == null && $sp['date2'] != null) { $sp['date1'] = $sp['date2']; $sp['date2'] = null; }
