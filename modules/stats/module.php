...
function stats_report_wimax_test($fixclient){
    ...
    include INCLUDE_PATH."1c_integration.php";
    $r = new \_1c\reports($db);
    //printdbg($r->test(3566, "2011-04-10 00:00:00", "2011-04-12 23:59:59"));
    //        exit();


    $pack = array(
            //"1" => array("item" => 0),
            "2" =>      array("item" => 3605, "descr_id" => "00000000-0000-0000-0000-000000000000","is_stat" => 0),
            "2-stat" => array("item" => 3605, "descr_id" => "00000000-0000-0000-0000-000000000000","is_stat" => 1),
            "3" =>      array("item" => 3566, "descr_id" => "00000000-0000-0000-0000-000000000000","is_stat" => 0),
            "3-stat" => array("item" => 3566, "descr_id" => "00000000-0000-0000-0000-000000000000","is_stat" => 1),
            "4" =>      array("item" => 4201, "descr_id" => "00000000-0000-0000-0000-000000000000","is_stat" => 0),
            "4-stat" => array("item" => 4201, "descr_id" => "00000000-0000-0000-0000-000000000000","is_stat" => 1),
            "5" =>      array("item" => 10935, "descr_id" => "00000000-0000-0000-0000-000000000000","is_stat" => 0),
            "5-stat" => array("item" => 10935, "descr_id" => "00000000-0000-0000-0000-000000000000","is_stat" => 1),
            );

    if(($listDate = get_param_raw("list", "")) !== "")
    {
        echo "<b>".$listDate."</b>";
        ...
        $b = array();
        if(file_exists("/tmp/save_test"))
        {
            $a = file_get_contents("/tmp/save_test");
            $a = unserialize($a);
            //printdbg($a);


            foreach($pack as $pId => $p)
            {
                if($p["is_stat"]) continue;

                foreach($a[$pId]["data"][$listDate]["orders"] as $o)
                {
                    $b[trim($o)] =1;
                }
            }
        ...
        foreach($db->AllRecords(
                    "select a.fio, l.bill_no, g.name, count(1) c
                    from tt_troubles t , tt_stages s, tt_stages s2, newbill_lines l, newbills b, g_goods g, newbills_add_info a
                    where s.date_start between '".$listDate." 00:00:00' and '".$listDate." 23:59:59'
                    and s.trouble_id = t.id and s.state_id = 18 and t.bill_no = l.bill_no and
                    item_id =g.id and  g.num_id in (3566)#3605,4201,10935)
                    and b.bill_no = l.bill_no and client ='WiMaxComstar' and a.bill_no = b.bill_no
                    and s2.stage_id =cur_stage_id and s2.state_id != 21
                    group by b.bill_no order by g.num_id, b.bill_no") as $idx => $v)
        {
            $isFound = isset($b[$v["bill_no"]]);
            unset($b[$v["bill_no"]]);
            //if($isFound)
            echo "<tr><td>".($counter++).". ".$v["fio"]."</td><td".($isFound ? " style='font-weight: bold;'" : "").">".$v["bill_no"]."</td><td>".$v["name"]."*".$v["c"]."</td></tr>";
        }
        foreach($b as $v => $k)
        {
            echo "<tr><td>...1c</td><td>".$v."</td><td></td></tr>";
            //
        }
        echo "</table>";
        echo "<hr>";
    ...
    $zero = strtotime(date("Y-m")."-01 00:00:00");

    $curDate = get_param_raw("date", $zero);
    $design->assign("curDate", $curDate);

    $design->assign("dates", array(
                strtotime("-6 month", $zero) => date("Y-m",strtotime("-6 month", $zero)),
                strtotime("-5 month", $zero) => date("Y-m",strtotime("-5 month", $zero)),
                strtotime("-4 month", $zero) => date("Y-m",strtotime("-4 month", $zero)),
                strtotime("-3 month", $zero) => date("Y-m",strtotime("-3 month", $zero)),
                strtotime("-2 month", $zero) => date("Y-m",strtotime("-2 month", $zero)),
                strtotime("-1 month", $zero) => date("Y-m",strtotime("-1 month", $zero)),
                $zero => date("Y-m",$zero)
                )
            );

    $typeId = "739a53ba-8389-11df-9af5-001517456eb1";
    $dateFrom = date("Y-m-d", $curDate);
    $dateTo = date("Y-m-d", strtotime("+1 month -1 day", $curDate));

    $d = array();

    $c = "<script src='js/jquery.js'></script>";

    foreach($pack as $pId => $p)
    {
        if($p["is_stat"])
        {
            $d[$pId]["data"] = $this->stat_test($p["item"], $dateFrom, $dateTo);
        }else{
            $d[$pId]["data"] = $r->test($p["item"], $dateFrom." 00:00:00", $dateTo." 23:59:59");
        }
        //printdbg($d[$pId]["data"]);
        $this->make_sum($d[$pId]);
        $d[$pId]["cost"] = $db->GetValue("select p.price from g_goods g, g_good_price p where g.id=p.good_id and price_type_id ='".$typeId."' and num_id = '".$p["item"]."' and descr_id = '".$p["descr_id"]."'");
        if(count($d[$pId]["data"]))
        {
            $dk = array_keys($d[$pId]["data"]);
            $d[$pId]["in"] = @array("start" => $d[$pId]["data"][$dateFrom]["start"], "end" => $d[$pId]["data"][$dateTo]["end"]);
        }else{
            $d[$pId]["in"] = array("start" => "???", "end" => "???");
        }
    }

    @unlink("/tmp/save_test");
    file_put_contents("/tmp/save_test", serialize($d));

    $c .= "<script>

        function ll(d)
        {
            $('#loadtd').load('./?module=stats&action=report_wimax_test&list='+d);
        }

        </script>";
        $c.= "<hr><table width=100%><tr><td valign=top>".$this->make_calend($curDate, $d)."</td><td id=loadtd valign=top align=left>11</td></tr></table>";

    $design->assign("data", $d);
    $design->assign("calend", $c);

    $design->AddMain("stats/wimax_test.tpl");
...
function stats_support_efficiency__basisOnCompleted(&$dateFrom, &$dateTo, &$usage)
{
    global $db;
/*
                    (select sum(rating) from tt_stages where trouble_id =tt.id and state_id=2) as rating_2,
                    (select sum(rating) from tt_stages where trouble_id =tt.id and state_id=7) as rating_7,
*/
