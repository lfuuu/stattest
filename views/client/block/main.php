    ...
    <div class="row">
        <div class="col-sm-6">
            <h2 class="c-blue-color no-margin">
                ...
                <small>(<?= $client->country->name ?>)</small>
            </h2>
        </div>
        <div class="col-sm-4" class="c-blue-color">
            <?php if (ApiCore::isAvailable()) : ?>
                <?php if ($client->isShowLkLink()) : ?>
                    <a href="https://<?= Yii::$app->params['CORE_SERVER']; ?>/core/support/login_under_core_admin?stat_client_id=<?= $client->id ?>&account_id=<?= $account->id ?>"
                       target="_blank">
                        Переход в ЛК
                    </a>
