            ...
            <table cellspacing="4" cellpadding="2" width="100%" border="0">
                ...
                <?php foreach ($services['ipport'] as $service): ?>
                    <tr bgcolor="<?= ($service->status == 'working') ? ($actual($service->actual_from, $service->actual_to) ? '#EEDCA9' : '#fffff5') : '#ffe0e0' ?>">
                        <td width="1%" nowrap="">
                            <a href="/pop_services.php?table=usage_ip_ports&id=<?= $service->id ?>" target="_blank">
                                <b><?= $service->id ?></b>
                            </a>
                        ...
                        <td><?= $service->actual_from ?>&nbsp;-&nbsp;<?= $service->actual_to ?></td>
                        <td>
                            <b>
                    ...
                    if ($service->net) :
                        foreach ($service->net as $net) :
                            ...
                            <tr bgcolor="<?= $actual($net->actual_from, $net->actual_to) ? '#DCEEA9' : '#fffff5' ?>">
                                ...
                                <td><?= $net->nat_net ? $ipstat($net->nat_net) : '' ?><?= $net->ip_net ? $ipstat($net->ip_net) : '' ?></td>
                                <td><?= $net->comment ?>}</td>
                            </TR>
                        <?php endforeach; ?>
                    <?php else : ?>
            ...
            <table class="table table-condensed">
                ...
                <?php foreach ($services['voip'] as $service): ?>
                    <tr bgcolor="<?= ($service->status == 'working') ? ($actual($service->actual_from, $service->actual_to) ? '#EEDCA9' : '#fffff5') : '#ffe0e0' ?>">
                        ...
                        <td>
                            <a href="/pop_services.php?table=usage_voip&id=<?= $service->id ?>"
                               target="_blank"><?= $service->actual_from ?>&nbsp;-&nbsp;<?= $service->actual_to ?></a>
                        </td>
                        ...
                        <td style="font-size: 8pt;"><?= $service->allowed_direction ?></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
        ...
        <div id="welltime">
            <table cellspacing="4" cellpadding="2" width="100%" border="0">
                ...
                <?php foreach ($services['welltime'] as $service): ?>
                    <tr bgcolor="<?= ($service->status == 'working') ? ($actual($service->actual_from, $service->actual_to) ? '#EEDCA9' : '#fffff5') : '#ffe0e0' ?>">
                        <td><a href="/pop_services.php?table=usage_welltime&id=<?= $service->id ?>"
                               target="_blank"><?= $service->actual_from ?>-<?= $service->actual_to ?></a>&nbsp;
