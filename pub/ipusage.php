<?php
	...
	$IP = array(
			'MCN-NET'		=> array('85.94.32.0','85.94.51.255',15,15),
			'MCN-ADSL-NET2'	=> array('85.94.54.0','85.94.62.255',15,6),
			'MCN-ADSL-NET3'	=> array('89.235.128.0','89.235.143.255',15,10),
			'MCN-ADSL-NET4'	=> array('89.235.144.0','89.235.159.255',15,10),
			'MCN-ADSL-NET5'	=> array('89.235.160.0','89.235.175.255',15,24),
			'MCN-ADSL-NET6'	=> array('89.235.176.0','89.235.191.255',15,15)
			);
	$IPs = get_param_raw('ip');
	$tmFrom = get_param_raw('from','2004-01-01');
	$tmTo = get_param_raw('to','2012-04-06');

	if ($image = get_param_integer('image',0)) {
		$tmFrom = strtotime($tmFrom);
		$tmTo = strtotime($tmTo);
		if ($IPs && isset($IP[$IPs])) {
			$IPs = array($IPs=>$IP[$IPs]);
		} else $IPs = $IP;
		...
		$tmTo = intval($tmTo/86400);
		    ...
		    $F = array(); $T = array();
		    while ($r = $db->NextRecord()) if (($v = netmask_to_ip_sum($r['net'])) && ($v[1]!=1)) {
			$b = null;
			foreach ($IPs as $ip) {
			    if ($v[0]>=$ip[0] && $v[0]<=$ip[1]) {$b=$ip; break;}
			}
			if ($b) {
		    ...
		    $D[count($D)-1] = $D[count($D)-1] & (~3);
		    if ($DMax>=$totalIPs) {
			foreach ($IPs as $k=>$ip) $IPs[$k][3]--;
		    }
		} while ((++$iter<20) && ($DMax>=$totalIPs)); 
