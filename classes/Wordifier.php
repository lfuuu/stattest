...
class Wordifier
{
    ...
    public static function Make($num, $currency)
    {
        ...
        $s .= Utils::rus_plural($v[1], self::$curBig[$currency][0], self::$curBig[$currency][1],
            self::$curBig[$currency][2]);
        $c = round(($num - floor($num)) * 100);
        $s .= ' ' . sprintf('%02d', $c) . ' ' . Utils::rus_plural($c, self::$curSmall[$currency][0],
                self::$curSmall[$currency][1], self::$curSmall[$currency][2]);
