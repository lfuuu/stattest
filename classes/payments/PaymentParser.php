            ...
            switch ($mode) {
                case 0:
                    foreach (PaymentParser::$types as $tkey => &$tval) {
                        if (stripos($line, $tkey) !== false) {
                            $type =& PaymentParser::$types[$tkey];
                            $mode = 1;
                            break;
                        }
                    }
                    break;
                case 1:
                    if (stripos($line, $type['pay_acc']) !== false) {
                        $mode = 12;
                        list(, $_payAcc) = explode("=", $line);
                        $payAcc[$_payAcc] = $_payAcc;
                    }
                    break;
                case 12:
                    if (stripos($line, $type['begin']) !== false) {
                        $mode = 3;
                        $C = array();
                    } else
                        if (stripos($line, $type['pay_acc']) !== false && stripos($line, "=") !== false) {
                            $mode = 12;
                            list(, $_payAcc) = explode("=", $line);
                            $payAcc[$_payAcc] = $_payAcc;
                        }
